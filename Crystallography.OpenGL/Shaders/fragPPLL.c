...
vec4 setColor()
{
    ...
    if (fWithTexture < 0.0)
    {
        ...
        float normalLengthSquared = dot(fNormal, fNormal); // (260320Ch) Line primitives can enter this path with a zero normal
        if (normalLengthSquared <= 1.0e-12)
        {
            c3 = inColor; // (260320Ch) Keep the requested line color instead of relying on undefined zero-normal lighting
        }
        else
        {
