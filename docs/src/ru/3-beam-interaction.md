...
|------|------|------|------|------|
| **X-ray** | структурные факторы, включая аномальную дисперсию | µ/ρ, µ, пропускание + края поглощения (в зависимости от энергии) | $f(s)$ (Waasmaier–Kirfel или Temari) или $F(q)+S(q)$ | характеристические линии + штрихи EDX |
| **Electron** | электронные структурные факторы | σ, MFP, \|dE/ds\|, IMFP, пробег (в зависимости от энергии) | Peng / Kirkland / 8-Gaussians / Temari | — (скрыто) |
...
## Вкладка Рефлексы {#reflections-tab}
...
=== "X-ray"
    ![Рефлексы — рентген](../assets/cap-ru-auto/FormBeamInteraction-xray-reflections.png)

=== "Electron"
    ![Рефлексы — электрон](../assets/cap-ru-auto/FormBeamInteraction-electron-reflections.png)

=== "Neutron"
    ![Рефлексы — нейтрон](../assets/cap-ru-auto/FormBeamInteraction-neutron-reflections.png)
...
=== "X-ray"
    ![Ослабление & перенос — рентген](../assets/cap-ru-auto/FormBeamInteraction-xray-attenuations.png)

=== "Electron"
    ![Ослабление & перенос — электрон](../assets/cap-ru-auto/FormBeamInteraction-electron-attenuations.png)

=== "Neutron"
    ![Ослабление & перенос — нейтрон](../assets/cap-ru-auto/FormBeamInteraction-neutron-attenuations.png)
...
## Вкладка Факторы рассеяния {#scattering-factors-tab}
...
- **X-ray** предлагает два режима **Model**: **f(s)** строит обычный рентгеновский атомный фактор рассеяния (в электронных единицах); **F(q)+S(q)** строит рэлеевский **когерентный** форм-фактор $F(q)$ вместе с комптоновской **некогерентной** функцией рассеяния $S(q)$ (из xraylib). Таблица также перечисляет члены аномальной дисперсии **f'(E)** и **f''(E)** при текущей энергии и столбец **модель**, указывающий, из какого источника получен столбец f(s). Когда выбрано **f(s)**, появляется вторая строка переключателей, задающая, **откуда берётся кривая**: **Waasmaier–Kirfel** (подгонка по умолчанию, охватывающая также ионы при Z = 1–98) или **Temari**. Она неактивна, пока показано **F(q)+S(q)**, поскольку эти две кривые берутся из xraylib и выбора источника не имеют.
- **Electron** предлагает четыре источника электронного фактора рассеяния: **Peng**, **Kirkland**, **8-Gaussians** и **Temari**. Таблица показывает $f_e(s)$ (nm) и какая **model** его породила.
...
- **Temari** — это не подгонка, а вычисленная таблица (Temari dataset-factors v1.0.0, CC-BY-4.0, из полного дираковского самосогласованного расчёта). Она охватывает **только нейтральные атомы Z = 1–86**: для ионов и Z = 87–98 записей нет, поэтому их кривые не строятся, а ячейки таблицы остаются пустыми вместо подстановки значений нейтрального атома. Пока выбран Temari, об этом сообщает строка-примечание.
- **Debye-Waller** умножает каждый фактор на тепловое затухание $e^{-B s^2}$, используя изотропный параметр смещения каждого атома.

---

## Вкладка Флуоресценция {#fluorescence-tab}
...
![Флуоресценция (рентген)](../assets/cap-ru-auto/FormBeamInteraction-xray-fluorescence.png)
