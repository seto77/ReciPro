...
- **Temari** — это не подгонка, а вычисленная таблица (Temari dataset-factors v1.0.0, CC-BY-4.0, из полного дираковского самосогласованного расчёта). Она охватывает **только нейтральные атомы Z = 1–86**: для ионов и Z = 87–98 записей нет, поэтому их кривые не строятся, а ячейки таблицы остаются пустыми вместо подстановки значений нейтрального атома.
