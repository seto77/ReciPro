...
|------|----------|------|
| **Рентгеновская (и нейтронная) дифракция** | Монокристальная картина рентгеновской / нейтронной дифракции (параллельная, прецессионная рентгеновская, Back Laue) | [Моделирование рентгеновской дифракции](5-x-ray-neutron-diffraction.md) |
...
> **ALCHEMI — не режим.** В отличие от рентгена / SAED / PED / CBED, которые определяются сочетанием длины волны и падающего пучка, симулятор ALCHEMI — это **дочернее окно**, открываемое из меню **Параметры**, поэтому здесь он приведён отдельно.

| Дочернее окно | Содержание | Страница |
|------|------|------|
| **Симулятор ALCHEMI** | Качательные кривые ионизации с разрешением по позициям (микроанализ с усилением каналированием) | [Моделирование ALCHEMI](4-alchemi-simulation.md) |
...
| Рентген | Параллельный | Рентгеновская дифракция | [Моделирование рентгеновской дифракции](5-x-ray-neutron-diffraction.md) |
| Рентген | Прецессия (рентген) | Прецессионная рентгеновская (прецессионная камера) | [Моделирование рентгеновской дифракции](5-x-ray-neutron-diffraction.md) |
| Рентген | Back Laue | Лауэ на отражение | [Моделирование рентгеновской дифракции](5-x-ray-neutron-diffraction.md) |
| Нейтрон | Параллельный | Нейтронная дифракция | [раздел о нейтронах в моделировании рентгеновской дифракции](5-x-ray-neutron-diffraction.md) |
...
| Клавиши-стрелки — **держатель ПЭМ** | Пошагово изменять наклон держателя (сначала отметьте **Включить клавиши со стрелками**; действует при любом окне ReciPro на переднем плане) |
...
| Получить монокристальную рентгеновскую дифракцию | Переключите **Wavelength** на рентген / синхротрон | [Моделирование рентгеновской дифракции](5-x-ray-neutron-diffraction.md) |
...
![Вкладка Кикучи (режим «Динамический»)](../../assets/cap-ru-auto/FormDiffractionSimulator.toolStripContainer1.panelMain.tabControl.tabPageKikuchi-dynamical.png)
...
- **Режим** : Способ отрисовки линий Кикучи. Настройки под полем со списком меняются вместе с режимом.
    - **Геометрия** : все выбранные линии одним фиксированным цветом.
    - **Кинематика** : темнота линии следует кинематической относительной интенсивности каждого отражения.
    - **Динамика** : двухволновые динамические профили полос избыток/дефицит с источником в виде термодиффузного рассеяния, вычисляемые в фоновом потоке. Полосы избытка появляются на стороне больших углов, дефицитные черты — в сторону прямого пучка; толщина образца общая с настройками интенсивности. Во время вычисления рисуются геометрические линии.
- **Настройки «Геометрия» / «Кинематика»** : **Цвет линий Кикучи** и **Толщина линии**.
- **Настройки «Динамика»** :
    - **Избыток** / **Дефицит** : цвета светлой (положительный контраст) и тёмной (отрицательный контраст) стороны полос.
    - **Полосы** : Число полос с динамическими профилями (лучшие *N* по динамическому контрасту; 1–50, по умолчанию 30).
    - **Качество** : Точки выборки поперёк каждой полосы (Быстро 65 / Стандарт 129 / Высокое 257).
    - **Контраст** : Усиление отображения контраста полос перед сжатием.
    - **Шкала** : Кривая сжатия яркости (Linear / Log / Tanh). Это визуализационные преобразования, а не физический отклик камеры.
    - **Фикс. яркость** : Фиксирует нормировку яркости, например для сравнения разных ориентаций или толщин.
    - Серая строка примечания показывает пометку о несогласованности источника и поглощения, число нарисованных полос и диагностику (например, подстановку *B*-фактора по умолчанию для атомов с *B* = 0).
...
- **Effective digit** : число значащих цифр, показываемых в таблице. **Auto resize row width** : автоподбор ширины столбцов. **Copy to clipboard** : экспортирует таблицу как текст, который можно вставить в электронную таблицу. **Save as CSV** : записывает ту же таблицу (видимые столбцы) в файл CSV; десятичный разделитель всегда точка, поэтому файл остаётся корректным и в локалях с десятичной запятой. (Эта форма отображается на английском даже при японском интерфейсе.)
...
Открывает дочернее окно, которое рассчитывает **качательные кривые ионизации с разрешением по позициям**: выход выбранного канала ионизации при наклоне кристалла через брэгговские условия систематического ряда, раздельно по атомным позициям. Открывается из меню **Параметры**. В отличие от окна CBED, оно имеет собственную область вывода и не рисует на холсте родительского окна.

→ Все настройки, а также область применимости и известные ограничения см. в разделе **[Моделирование ALCHEMI](4-alchemi-simulation.md)**.
...
- [Моделирование ALCHEMI](4-alchemi-simulation.md)
