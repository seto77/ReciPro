...
Интенсивность определяется исключительно по геометрическому расстоянию между сферой Эвальда и узлом обратной решётки (ошибке возбуждения $S_g$). Она максимальна при $S_g = 0$ — точно на сфере — и убывает с ростом $\lvert S_g\rvert$, обращаясь в нуль, как только $\lvert S_g\rvert$ превышает **Radius**, заданный для узла обратной решётки. Поскольку структурный фактор кристалла игнорируется, это самый быстрый режим, подходящий для проверки положений дифракционных рефлексов.
...
| **2θ** | угол рассеяния $2\theta$ (то же определение, что и у концентрических окружностей шкалы 2θ) |
