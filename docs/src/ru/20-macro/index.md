...
На приведённом выше снимке экрана включён режим **Показать примеры**, отображающий встроенные образцы макросов. Список макросов находится слева, редактор кода — справа, а таблица справки по встроенным функциям — внизу.
...
При включении **Показать примеры** (вверху слева) ваш список макросов временно заменяется встроенными образцами макросов (простые циклы и условия, математические функции, поворот/выравнивание, обход списка кристаллов, симуляция дифракции/изображений, серии по наклону/энергии, сведения о рефлексах и многое другое). Образцы доступны только для чтения и отображаются на текущем языке интерфейса; используйте их для обучения или как отправную точку для копирования. При отключении восстанавливаются ваши собственные макросы.
...
ReciPro.Crystal.*      # Текущий кристалл: чтение, создание, редактирование
...
ReciPro.SpotID.*       # Spot ID (v2): поиск рефлексов и определение ориентации
ReciPro.StructureViewer.*  # Просмотрщик структуры: экспорт изображений и 3D-моделей
