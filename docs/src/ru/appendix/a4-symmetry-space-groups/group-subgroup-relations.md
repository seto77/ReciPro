...
## Вкладка «Элементы и позиции»

Там, где вкладка **Матрица** перечисляет сохранённые и утраченные операции в виде таблицы, эта вкладка показывает ту же информацию *геометрически* — в двух рисунках, которые кристаллографы и так привыкли читать бок о бок в ITA Vol. A: слева — [диаграмма элементов симметрии](symbols-and-diagrams.md#symmetry-element-diagram) родителя (оси, плоскости и центры инверсии); справа — **диаграмма общих позиций** родителя. Оба следуют одному цветовому правилу — **то, что сохраняется в $H$, — чёрное, а то, что утрачено, — жёлтое**, при этом элементарная ячейка под ними нарисована синим, — поэтому вкладка с первого взгляда отвечает: какие элементы симметрии разрушаются и как расщепляется орбита общей позиции, когда кристалл переходит из $G$ в подгруппу $H$. Направление проекции (⟂ *a*, *b* или *c*) выбирается автоматически по кристаллической системе родителя, а заголовок называет отношение, его вид и индекс, а также проекцию.

**Слева — элементы симметрии.**

- Наложение рисует чёрным элементы симметрии, реконструированные напрямую из собственного набора операций $H$, поверх утраченных элементов родителя, нарисованных жёлтым, — так что сохранёнными оказываются именно те элементы, которые вновь появляются в $H$, без посимвольного угадывания.
- Элемент симметрии, вырождающийся в более низкий, показывается одним лишь уцелевшим символом: там, где ось 4-го порядка сохраняет лишь свою ось 2-го порядка, появляется просто **чёрный символ оси 2-го порядка**. Утраченный элемент печатается жёлтым только там, где в его геометрическом месте не выживает ничего, — так уцелевшая симметрия остаётся читаемой.

Левый рисунок обрабатывает три ситуации в зависимости от того, как ячейка подгруппы связана с ячейкой родителя:

- **Та же условная ячейка** — *translationengleiche* (*t*-) подгруппы, а также *klassengleiche* (*k*-) подгруппы, которые лишь удаляют векторы центрирования ($\det$ базиса подрешётки $= 1$). Здесь элементы $H$ живут в ячейке родителя, поэтому наложение — это рисунок одной родительской ячейки. (Отношение *k*-, удаляющее центрирование, например $I$- или $F$-центрированная $\to$ примитивная, показывает, как винтовые оси и плоскости скольжения, порождённые центрированием, становятся жёлтыми.)
- **Увеличение ячейки в плоскости** — отношения *k*- или изоморфные, увеличение которых лежит в плоскости *a*–*b* ($a'=n_a a$, $b'=n_b b$, при неизменном *c*) в ромбическом, тетрагональном или кубическом родителе. Увеличенная ячейка рисуется (проекция ⟂ *c*) как сетка $n_a\times n_b$ из плиток родительской ячейки, и **каждая плитка окрашивается независимо**: элемент симметрии, сохранённый в $H$, чёрный в тех плитках, где эта копия уцелела, и жёлтый там, где увеличение ячейки его удалило, — так удвоение ячейки видно прямо по тому, как жёлтой становится каждая вторая копия.
- **Иначе** (увеличение вдоль оси наблюдения $c$, косоугольные/неортогональные изменения ячейки, гексагональные/тригональные/моноклинные родители) утраченную симметрию нельзя нарисовать однозначно в 2-D проекции, поэтому вкладка показывает короткую заметку, отсылающую к вкладкам **Домены и двойники** и **Новые отражения**, которые уже несут информацию об утраченной симметрии решётки для этих случаев.

**Справа — общие позиции.** Под действием $G$ все нарисованные точки образуют одну орбиту (одну общую позицию); под действием $H$ эта орбита расщепляется на $[G:H]$ подорбит (сравните с вкладкой **Расщепление орбит**). Подорбита, содержащая представительную точку, — множество, остающееся одной общей позицией $H$, — рисуется **чёрным**; точки остальных подорбит, переставшие быть эквивалентными ей после понижения симметрии, — **жёлтые**. Там, где проекция совмещает две эквивалентные точки, используется стандартный для ITA *разделённый кружок*: круг, разделённый вертикальной линией, с запятой, помечающей зеркально-отражённую половину, и меткой высоты (`+`, `½−`, …) возле каждой половины. **Если одна половина выживает в $H$, а другая утрачивается, разделяющая линия, а также запятая и метка высоты утраченной половины рисуются жёлтым** — так наполовину утраченную позицию можно прочитать прямо с рисунка. Раскраска орбит показывается для отношений с той же ячейкой (*t*-, а также удаляющих центрирование *k*-); для отношений с увеличением ячейки правая панель вместо этого показывает короткую заметку.

Для R-пространственных групп, отображаемых в ромбоэдрической установке осей (Rho), ни один из двух рисунков нельзя построить в этой проекции; вместо них вкладка показывает заметку, а обе диаграммы доступны в гексагональной установке осей (Hex) той же R-пространственной группы.

![Вкладка «Элементы и позиции»](../../../assets/cap-ru-auto/FormGroupRelations-tabElements.png)
