...
Для перехода в *t*-подгруппу перечисляет отражения (поиск до $|h|,|k|,|l|\le4$), которые становятся разрешёнными симметрией в $H$, хотя были систематически погашены в $G$, — т. е. отражения, которые запрещены условиями отражения родителя (со вкладки [Условия](../../2-symmetry-information.md)), но не условиями $H$.
...
Для ***k*- или изоморфного отношения** вкладка перечисляет новые отражения, **индексированные в увеличенной ячейке подгруппы** (снова до $|h'|,|k'|,|l'|\le4$), и классифицирует каждое из них в последнем столбце:
...
- **Изоморфные подгруппы перечисляются только до индекса ≤ 4.** Изоморфная серия продолжается неограниченно к более высоким индексам (для кубической группы уже $a'=3a$ — это индекс 27), поэтому дерево помечает ветвь серой заметкой *«только индекс ≤ 4 — изоморфные серии продолжаются при больших индексах»*, а не делает вид, что список полон.
...
- Окно поиска отражений фиксировано на $|h|,|k|,|l|\le4$ и в текущей версии пользователем не настраивается.
