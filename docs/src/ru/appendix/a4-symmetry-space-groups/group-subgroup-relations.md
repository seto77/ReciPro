...
Эта вкладка обрабатывает три ситуации в зависимости от того, как ячейка подгруппы связана с ячейкой родителя:

- **Та же конвенциональная ячейка** — *translationengleiche* (*t*-) подгруппы, а также *klassengleiche* (*k*-) подгруппы, которые лишь удаляют векторы центрирования (определитель базиса подрешётки $= 1$). Здесь элементы $H$ живут в ячейке родителя, поэтому наложение — это рисунок одной ячейки. (Отношение *k*-, удаляющее центрирование, например $I$- или $F$-центрированная $\to$ примитивная, делает красными винтовые оси и плоскости скольжения, порождённые центрированием.)
- **Увеличение ячейки в плоскости** — отношения *k*- или изоморфные, увеличение которых лежит в плоскости *a*–*b* ($a'=n_a a$, $b'=n_b b$, при неизменном *c*) в ромбическом, тетрагональном или кубическом родителе. Увеличенная ячейка рисуется (проекция ⟂ *c*) как сетка $n_a\times n_b$ из плиток родительской ячейки, и **каждая плитка окрашивается независимо**: элемент симметрии, сохранённый в $H$, чёрный в тех плитках, где эта копия уцелела, и красный там, где увеличение ячейки его удалило — так удвоение ячейки видно напрямую как каждая вторая копия, становящаяся красной.
