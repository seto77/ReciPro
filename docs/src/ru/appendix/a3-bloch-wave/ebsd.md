...
### Глубина источника и длина пути

По теореме взаимности master pattern для направления $\widehat{\mathbf s}$ рассчитывается как задача о плоской волне, входящей вдоль $-\widehat{\mathbf s}$, причём для каждого направления поверхность кристалла считается перпендикулярной $\widehat{\mathbf s}$; поэтому его координата толщины $t$ отсчитывается вдоль направления выхода. Глубина источника $z$ из расчёта Монте-Карло, напротив, отсчитывается вдоль нормали к наклонённой поверхности образца. При $\mu=\cos\chi$, где $\chi$ — угол между направлением выхода и нормалью к поверхности, источник на глубине $z$ находится вдоль направления выхода на длине пути

$$t=\frac{z}{\mu}$$

от поверхности. ReciPro аппроксимирует распределение по глубине для каждого бина направления выхода и каждой энергии экспонентой $\propto e^{-z/\lambda_z(E)}$, пересчитывает его в каждом пикселе детектора с $\mu$ этого пикселя в $\lambda_t=\lambda_z/\mu$ и интегрирует полученное распределение, нормированное в пределах массива глубин, по каждому интервалу толщины master pattern. Поэтому в приведённой выше сумме индекс глубины пробегает толщины $t_j$, а вес пикселя зависит не только от $\widehat{\mathbf s}$, но и от его $\mu$. При скользящем выходе (малое $\mu$) источник на той же глубине проходит более длинный путь, и вес смещается к более толстым слоям.

Автоматический массив глубин заканчивается на 99,9-м процентиле длины пути $z/\mu$ электронов, достигающих детектора (с округлением вверх до двух значащих цифр), и содержит 40 точек, расположенных гуще вблизи поверхности.
