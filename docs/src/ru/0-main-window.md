...
Главное окно устанавливает несколько **общеприложенческих** сочетаний клавиш. Они продолжают работать, пока в фокусе находятся окна Просмотр структуры, Стереосеть, Симулятор дифракции и Spot ID.
...
| Пункт меню | Описание |
|-----------|-------------|
| Показывать всплывающие подсказки | Переключить отображение всплывающих подсказок |
| Тёмная тема | Переключение между светлой и тёмной цветовыми темами |
| Использовать индексы Миллера–Браве (hkil) | Использовать 4-индексную нотацию для тригональных/гексагональных систем во всём приложении |
| Использовать ионные факторы рассеяния | Учитывать вклад ионного заряда в атомные факторы рассеяния, используемые в расчётах |
| Скачать библиотеку MKL | Скачать нативную библиотеку Intel MKL (~55 МБ); после установки она автоматически используется для больших вычислений |
| Accept external macro commands (named pipe) | Слушать именованный канал `ReciPro.Macro.v1` и выполнять макросы, присланные внешними программами — см. [Внешнее управление](20-macro/3-external-control.md) |
| Сбросить настройки реестра при выходе (вступает в силу после перезапуска) | Сбросить настройки при следующем перезапуске |
| Отключить библиотеку Crystallography.Native (требуется перезапуск) | Перейти на управляемый код, если нативная (C++) библиотека не загружается |
| Отключить весь рендеринг OpenGL (требуется перезапуск) | Для старых GPU / удалённого рабочего стола |
| Отключить рендеринг текста OpenGL (требуется перезапуск) | Обходное решение для проблем отрисовки текста на некоторых GPU |
| Захватить компоненты GUI... | Инструмент разработчика для сохранения скриншотов GUI |
| Функция порошковой дифракции (в разработке) | Включить окно поликристаллической (порошковой) дифракции |
...
Язык интерфейса переключается через отдельное меню **Язык** (требуется перезапуск).

### Язык

Переключение языка интерфейса. Доступны одиннадцать языков — English, 日本語, Deutsch, Français, Español, Português, Italiano, Русский, 简体中文, 繁體中文 и 한국어 — каждый показан в собственной письменности. Изменение вступает в силу после перезапуска ReciPro. Онлайн-руководство следует той же настройке.
...
Верхняя часть панели всегда показывает **Name** (название кристалла), **Формула** (химическая формула, вычисленная из списка атомов) и **Z** (число формульных единиц на элементарную ячейку). Кнопки **Инфо о симметрии** и **Взаимодействие пучка** открывают окна [Сведения о симметрии](2-symmetry-information.md) и [Взаимодействие пучка](3-beam-interaction.md).
...
| Элемент | Описание |
|------|------|
| Параметры ячейки | Параметры решётки a, b, c (**Ед.**: Å = 10⁻¹⁰ м или nm) и α, β, γ. Выбор симметрии автоматически ограничивает их (например, a=b=c, α=β=γ=90° для кубической). |
| Симметрия | Выберите **Крист. система**, **Точечная гр.** и **Простр. группа**. Введите текст в поле **Поиск**, чтобы перечислить подходящие кандидаты (с учётом регистра). |
| Объём ячейки / Масса ячейки | Объём и масса элементарной ячейки. Объём отображается в Å³ / nm³ в соответствии с выбором **Ед.** |
| Молярный объём / Молярная масса / Плотность | Молярный объём, молярная масса и плотность. Показываются **только при введённых атомах**. |
| Среднее Z (число) / Среднее Z (масса) / Среднее A | Средний атомный номер, взвешенный по числу и по массе, и средний атомный вес на атом. |
| Атомная плотность / Электронная плотность | Число атомов и электронов на объём ячейки; отображается в Å⁻³ / nm⁻³ в соответствии с выбором **Ед.** |
...
Щёлкните правой кнопкой мыши по строке атома и выберите **Показать эквивалентные позиции атома**, чтобы открыть небольшое окно со списком всех позиций, порождённых операциями симметрии пространственной группы; кнопка **Copy** копирует список как текст, разделённый табуляциями (например, для вставки в электронную таблицу).
...
| Powder Diffraction | Поликристаллическая (порошковая) дифракция — включить через **Параметры ▸ Функция порошковой дифракции (в разработке)** | - |
