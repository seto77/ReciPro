...
Язык переключается через отдельное меню **Language** (английский/японский, требуется перезапуск).
...
![Просмотр вдоль](../assets/cap-ru-auto/FormMain.toolStripContainer1.panel1.groupBoxProjectAlong.png)
...
| Ось зоны | Вы знаете направление обзора, например \([001]\) или \([110]\). | **Просмотр вдоль** / ввод оси зоны |
