...
上方的螢幕擷圖已開啟 **Show samples**，顯示內建的範例巨集。巨集清單在左側、程式碼編輯器在右側，底部則是內建函式的說明表。
...
開啟 **Show samples**（左上角）會以內建的範例巨集暫時取代您的巨集清單（基本迴圈與條件式、數學函式、旋轉/對齊、掃描晶體清單、繞射/影像模擬、傾斜/能量序列、反射資訊等等）。這些範例為唯讀，並以目前的 UI 語言顯示；可用於學習，或作為複製的起點。將其關閉即會還原您自己的巨集。
