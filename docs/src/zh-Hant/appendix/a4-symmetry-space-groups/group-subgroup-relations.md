...
**變換矩陣**索引標籤以表格列出保持與消失的操作，本索引標籤則把相同的資訊以*幾何方式*呈現，就在結晶學家早已於 ITA Vol. A 中並排閱讀的兩張圖裡：左側是母群的[對稱元素示意圖](symbols-and-diagrams.md#symmetry-element-diagram)（軸、面與反演中心）；右側是母群的**一般位置圖**。兩張圖遵循同一條顏色規則 — **在 $H$ 中存續者為黑色，消失者為黃色**，其下並以藍色繪出晶胞 — 因此本索引標籤一眼就回答了：當晶體從 $G$ 轉變為子群 $H$ 時，哪些對稱要素破缺、一般位置的軌道又如何分裂。投影方向（⟂ *a*、*b* 或 *c*）依母群的晶系自動選定，標頭並標明該關係、其種類與指數，以及投影方向。

**左 — 對稱要素**

- 重疊圖的作法，是在以黃色繪出之母群消失要素的上方，以黑色疊繪直接由 $H$ 自身操作集合重建而得的對稱要素 — 因此保持的要素正是那些在 $H$ 中重新出現者，無需逐一符號臆測。
- 對稱要素退化為較低者時，僅以存續的記號表示：例如某條四重軸只保留其二重軸之處，就只出現**黑色的二重符號**。消失的要素唯有在其幾何位置上一無所存時才以黃色印出，讓存續的對稱保持易讀。

左圖依子群晶胞與母群晶胞的關係，處理三種情形：

- **相同慣用晶胞** — *translationengleiche*（*t*-）子群，以及僅移除心化向量的 *klassengleiche*（*k*-）子群（子點陣基底之 $\det = 1$）。此時 $H$ 的要素落在母群晶胞內，故重疊圖為單一母胞的圖。（移除心化的 *k*- 關係，例如 $I$ 心或 $F$ 心 $\to$ 簡單格子，會使心化所生成的螺旋軸與滑移面變黃。）
- **面內晶胞擴大** — 擴大位於 *a*–*b* 面內（$a'=n_a a$，$b'=n_b b$，*c* 不變）的斜方、正方或立方母群的 *k*- 或同型關係。擴大後的晶胞（投影 ⟂ *c*）繪製為 $n_a\times n_b$ 個母胞方塊的網格，且**每塊方塊獨立著色**：在 $H$ 中保持的對稱要素，於該副本得以留存的方塊中為黑色，於晶胞擴大已將其移除處為黃色 — 於是晶胞倍增直接表現為每隔一個副本變黃。
- **其他情形**（沿視線軸 $c$ 的擴大、斜交／非正交的晶胞變換、六方／三方／單斜母群）失去的對稱無法在二維投影中無歧義地繪製，故索引標籤顯示一段簡短說明，指向**疇與雙晶**與**新反射**索引標籤 — 這兩者對該類情形已載有失去之點陣對稱性的資訊。

**右 — 一般位置**：在 $G$ 之下，所有繪出的點構成單一軌道（一個一般位置）；在 $H$ 之下，該軌道分裂為 $[G:H]$ 個副軌道（可與**軌道分裂**索引標籤對照）。包含代表點的副軌道 — 即仍作為 $H$ 之一個一般位置留存的集合 — 繪為**黑色**；其他副軌道的點在對稱性降低後不再與之等價，則為**黃色**。當投影使兩個等價點重合時，採用 ITA 標準的*分裂圓*（split circle）：以一條垂直線一分為二的圓，鏡像的那一半標以逗號，各半旁並附高度標記（`+`、`½−`、…）。**若一半在 $H$ 中存續而另一半消失，分隔線與消失那一半的逗號及高度標記便以黃色繪出**，因此「半消失」的位置可直接從圖上讀出。軌道著色僅對同胞關係（*t*-，以及移除心化的 *k*-）顯示；對擴大晶胞的關係，右側面板改為顯示一段簡短說明。

對以菱面體軸 (Rho) 設定顯示的 R 空間群，兩張圖在此投影下皆無法繪製；索引標籤會改為顯示一段註記，兩張圖皆可在同一 R 空間群的六方軸 (Hex) 設定下顯示。

![對稱要素與一般位置索引標籤](../../../assets/cap-zh-Hant-auto/FormGroupRelations-tabElements.png)
