...
本索引標籤**僅對 *translationengleiche*（*t*-）子群提供**，此時 $H$ 保留母群的平移點陣（$T_H=T_G$），故兩群的要素同處於同一晶胞中。對 *k*- 與同型關係，晶胞會擴大，此圖留待後續版本；此時索引標籤顯示一段簡短說明，指向**疇與雙晶**與**新反射**索引標籤 — 這兩者對該類情形已載有失去之點陣對稱性的資訊。
