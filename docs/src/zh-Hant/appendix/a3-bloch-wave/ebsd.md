...
EBSD（電子背向散射繞射）使用與 CBED 和 STEM 相同的 Bethe/布洛赫波核心，但問題的提法不同。CBED 和 STEM 是**入射束問題**：一束電子波從試樣外部進入，並計算其出射波。EBSD 是**出射方向問題**：在試樣內部經歷了非彈性散射的電子作為背向散射電子射出，計算所要回答的是有多少強度沿每個外部方向離開。
...
master pattern 本身並不是偵測器影像；它是晶體固定方向空間中的強度分布。ReciPro 用等面積的 Roşca–Lambert 投影對該方向空間進行取樣，並將 $+Z$ 與 $-Z$ 兩個半球儲存為各自獨立的平面陣列。等面積取樣減小了兩極與赤道之間的密度偏差。
...
根據相反定理，方向 $\widehat{\mathbf s}$ 的 master pattern 被當作沿 $-\widehat{\mathbf s}$ 入射的平面波問題求解，並且對每個方向都把晶體表面視為垂直於 $\widehat{\mathbf s}$。因此其厚度座標 $t$ 是沿出射方向量測的長度。與此不同，蒙地卡羅給出的源深度 $z$ 是沿傾斜試樣表面的法線方向量測的深度。設出射方向與表面法線的夾角為 $\chi$、$\mu=\cos\chi$，則深度為 $z$ 的源沿出射方向到表面的路徑長度為

$$t=\frac{z}{\mu}$$

ReciPro 對每個出射方向的分箱與每個能量，以指數分布 $\propto e^{-z/\lambda_z(E)}$ 擬合深度分布，在偵測器的每個像素上以該像素的 $\mu$ 換算為 $\lambda_t=\lambda_z/\mu$，再將在深度陣列範圍內正規化的分布依 master pattern 的每個厚度區間積分，作為權重。因此上式求和中的深度下標遍歷厚度 $t_j$，像素的權重不僅取決於 $\widehat{\mathbf s}$，也取決於該像素的 $\mu$。出射越掠射（$\mu$ 越小），同一深度的源經過的路徑越長，權重便移向更厚的切片。

自動設定的深度陣列以到達偵測器之電子的路徑長度 $z/\mu$ 的第 99.9 百分位數（無條件進位至 2 位有效數字）為上限，由越靠近表面越密的 40 個不等間距點組成。
