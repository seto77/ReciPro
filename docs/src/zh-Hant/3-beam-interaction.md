...
- **X-ray** 提供兩種 **Model** 模式：**f(s)** 繪製傳統的 X 射線原子散射因子 (以電子單位)；**F(q)+S(q)** 繪製 Rayleigh **同調**形狀因子 $F(q)$ 連同 Compton **非同調**散射函數 $S(q)$ (來自 xraylib)。表格還列出當前能量下的異常色散項 **f'(E)** 與 **f''(E)**。 選擇 **f(s)** 時會出現第二列選項按鈕，用於選擇**曲線的資料來源**——**Waasmaier–Kirfel**（預設的擬合，且涵蓋 Z = 1–98 的離子）或 **Temari**。在顯示 **F(q)+S(q)** 期間該列呈灰色，因為這兩條曲線來自 xraylib，沒有來源可選。
