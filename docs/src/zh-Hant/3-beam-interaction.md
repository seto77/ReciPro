...
| 射束 | 繞射 | 衰減 & 輸運 | 散射因子 | 螢光 |
|------|------|------|------|------|
| **X-ray** | 含異常色散的結構因子 | µ/ρ、µ、透射率 + 吸收邊 (對能量) | $f(s)$（Waasmaier–Kirfel 或 Temari）或 $F(q)+S(q)$ | 特徵譜線 + EDX 棒狀圖 |
| **Electron** | 電子結構因子 | σ、MFP、\|dE/ds\|、IMFP、射程 (對能量) | Peng / Kirkland / 8-Gaussians / Temari | —(隱藏) |
...
## 散射因子 索引標籤 {#scattering-factors-tab}
...
- **X-ray** 提供兩種 **Model** 模式：**f(s)** 繪製傳統的 X 射線原子散射因子 (以電子單位)；**F(q)+S(q)** 繪製 Rayleigh **同調**形狀因子 $F(q)$ 連同 Compton **非同調**散射函數 $S(q)$ (來自 xraylib)。表格還列出當前能量下的異常色散項 **f'(E)** 與 **f''(E)**，以及標明 f(s) 欄來自哪個資料來源的 **模型** 欄。 選擇 **f(s)** 時會出現第二列選項按鈕，用於選擇**曲線的資料來源**——**Waasmaier–Kirfel**（預設的擬合，且涵蓋 Z = 1–98 的離子）或 **Temari**。在顯示 **F(q)+S(q)** 期間該列呈灰色，因為這兩條曲線來自 xraylib，沒有來源可選。
- **Electron** 提供電子散射因子的四種來源：**Peng**、**Kirkland**、**8-Gaussians** 與 **Temari**。表格顯示 $f_e(s)$ (nm) 及產生它的 **model**。
- **Neutron** 散射長度不依賴 $s$，因此不繪製曲線；表格列出各元素的同調散射長度 **b_coh** 及其同調 / 非同調截面。
- **Temari** 不是擬合，而是計算得到的表格（Temari dataset-factors v1.0.0，CC-BY-4.0，來自完全 Dirac 自洽場計算）。它**僅涵蓋中性原子 Z = 1–86**：離子與 Z = 87–98 沒有對應數值，因此不繪製其曲線，並將表格儲存格留空，而非以中性原子數值填入。選擇 Temari 期間會顯示一行提示說明這一點。
- **Debye-Waller** 使用各原子的等向位移參數，以熱阻尼 $e^{-B s^2}$ 乘以每個因子。

---

## 螢光 索引標籤 {#fluorescence-tab}
