...
- **對比隨能量損失衰減**（預設開啟，E_c = 0.8 keV）：將每個能量切片的菊池對比乘以 A(E) = exp(−(E0 − E)/E_c)（E0 為入射能量，E 為出射能量），並把損失的部分轉為依方向平均的平坦基座，因此去掉帶對比後的亮度不變。蒙地卡羅權重逐一計數背向散射電子，但損失大量能量的電子幾乎不帶有銳利的菊池對比；關閉此修正時，所有切片都帶滿額對比，帶比 E0 的單色電子寬約 12 %（Si，20 kV）。預設值 0.8 keV 是以一張 Si 20 kV 實測圖樣擬合帶寬校準的值，是否適用於其他物質或加速電壓尚未確認。修改該值只會對現有切片重新加權，不會重新計算 master pattern 或蒙地卡羅。
