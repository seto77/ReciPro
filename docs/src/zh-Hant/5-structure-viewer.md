...
|----------|------|
| 儲存影像 | 將算繪影像儲存到檔案 |
| 複製主影像至剪貼簿 | 複製影像（也可用 Ctrl+Shift+C） |
| 儲存影片 | 將旋轉/平移影片儲存為 MP4（可設定速度、長度、fps、品質等） |
| Export 3D Model (3MF/STL) | 將顯示中的結構輸出為可用於 3D 列印的立體資料 |
| 快速鍵提示 | 顯示鍵盤快速鍵 |
...
**Export 3D Model (3MF/STL)** 會將畫面上目前繪製的結構模型輸出為可用於 3D 列印的立體資料。只有封閉實體才能輸出——原子（球、橢球）、鍵（圓柱）與配位多面體。原子標籤、邊界面與晶面無法列印，因此會被忽略。晶胞外框由線段構成，只有勾選對應選項時才會替換成圓柱（角落加球）後輸出。

![Export 3D Model 對話方塊](../assets/cap-zh-Hant-auto/FormExport3DModel.png)

對話方塊上方會顯示參與輸出的物件數與三角形數，以及模型的實際尺寸（**Model size**，nm）。

**Scale** : 輸出尺寸的決定方式

- **Fit largest dimension** : 自動決定比例，使模型的最長邊達到指定長度（5–500 mm，預設 80 mm）。
- **Fixed scale** : 直接指定比例（mm/nm）。用於以相同比例製作多個晶體並排比較。切換到此項時，會以剛才生效的比例作為初始值。
- 下方一行會顯示決定後的比例與輸出尺寸（**Output size**，mm）。

**Include** : 列印中要包含的要素

- **Atoms** / **Bonds** / **Polyhedra** : 只能選擇目前顯示的種類；取消勾選後仍留在畫面上，但不會進入列印。
- **Polyhedra: solid faces** / **edges, dia.** : 選擇要將配位多面體輸出為有面的實心體，或僅稜邊的骨架（圓柱＋頂點球，預設 2.0 mm）。骨架可以看見內部的原子。
- **Unit cell edges as cylinders, dia.** : 將晶胞外框替換為指定直徑（預設 2.4 mm）的圓柱後納入。

**Printability** : 列印適性

- **Thicken bonds thinner than** : 列印後直徑低於指定值（預設 1.2 mm）的鍵，會依原始圖元以該直徑重新產生，以免細鍵折斷。若關閉此項而存在過細的鍵，則會出現紅色警告。

**Format** : 檔案格式

- **STL (single color)** : 單色二進位 STL。
- **3MF (parts colored by element)** : 依元素著色的 3MF；顏色名稱中帶有元素名。

按 **Save...** 會詢問檔案名稱。輸出結束後，三角形數、輸出尺寸與比例會記錄在左下角的資訊欄中。
