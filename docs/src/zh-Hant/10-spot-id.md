...
Spot ID v1 以**數值輸入**的方式接收斑點幾何（即先前的 *TEM ID* 工作流程），而斑點偵測／擬合則透過按鈕操作；繞射影像僅供參考顯示，無法點按互動（滑鼠縮放與手動挑選斑點屬於 [Spot ID v2](11-spot-id-v2.md)）。唯一的快速鍵位於結果視窗：
...
→ 各視窗一覽請參見 **[21. 鍵盤與滑鼠快速鍵](21-shortcuts.md)**。
