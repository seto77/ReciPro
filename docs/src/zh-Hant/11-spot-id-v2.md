...
**儲存 ▸ Candidate list (CSV)...** 可匯出指標化得到的取向候選，**複製 ▸ Candidate list (TSV)** 則把同一表格放入剪貼簿：每個候選 × 晶粒一列，包含排名、晶粒序號、晶體名稱、ZXZ 尤拉角（°）、旋轉矩陣的九個分量、均方殘差（nm⁻²）、已指派繞射斑數以及繞射斑 → (hkl) 的對應關係。CSV 的小數點一律為句點；TSV 可直接貼入試算表。
