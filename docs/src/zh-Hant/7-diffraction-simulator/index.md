...
| 方向鍵 — **TEM holder** | 逐步調整試樣台傾斜（先勾選 **啟用方向鍵**；無論 ReciPro 的哪個視窗在前景均有效） |
...
開啟一個將繞射圖樣與雙傾（或旋轉）**TEM 試樣台** 連動的視窗。設定試樣台傾斜角會更新圖樣與晶體取向，且可達的取向可在極網上顯示（v4.914 新增）。在極網上左鍵雙擊會將試樣台傾斜設為該點，而勾選 **啟用方向鍵** 可讓方向鍵逐步調整傾斜。只要此視窗處於開啟狀態，無論 ReciPro 的哪個視窗位於前景，方向鍵均有效。
...
- **Effective digit** ：表格中顯示的有效位數。**Auto resize row width** ：自動調整欄寬。**Copy to clipboard** ：將表格匯出為可貼入試算表的文字。**Save as CSV** ：將同一表格（可見欄）寫入 CSV 檔；小數點一律為句點，因此在以逗號作小數點的地區設定下檔案仍然有效。（即使在日文介面下，此表單仍以英文顯示。）
