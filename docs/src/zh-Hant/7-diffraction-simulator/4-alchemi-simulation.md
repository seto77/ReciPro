...
| 項目 | 說明 | 預設值 |
|------|------|--------|
| **反射列 g = ( h k l )** | 要掃描的系統反射列，以其倒易晶格向量 $\mathbf{g} = h\mathbf{a}^* + k\mathbf{b}^* + l\mathbf{c}^*$ 的反射指數 $(h\,k\,l)$ 給定（不是方向指數 $[u\,v\,w]$）。傾轉軸取為同時垂直於束流與此 $\mathbf{g}$，因此掃描會帶著該反射列通過其 Bragg 條件 | (1 0 0) |
...
給定起點、終點與步長（nm）。**所有厚度在一次執行中一併算出**，結果以曲線下方的**厚度**欄切換（微調按鈕在已算出的厚度間逐級切換；鍵入的值會對齊到最近的厚度）。若起點與終點只給出一個厚度，則沒有可切換的對象，該欄將被停用。
...
**1600 波的上限與游離形狀因子的收錄範圍 $s \le 16\ \text{Å}^{-1}$ 是成對的。** 實測顯示即使 1600 波，基組所需的 $s$ 也僅約 10.5 Å⁻¹，因此只要遵守此上限就不會用盡收錄範圍。實際達到的數值顯示於圖下方[基組診斷](#基組診斷)框的第一列。
...
| 項目 | 說明 |
|------|------|
| **厚度** | 選擇顯示的厚度；微調按鈕在已算出的厚度間逐級切換，鍵入的值會對齊到最近的厚度（不會重新計算） |
...
曲線下方唯讀診斷框（其餘內容可捲動檢視；文字可選取複製）的末尾幾列依系列給出**對比度** $(\max-\min)/\text{mean}$ 以及相對於首個系列的**相關係數** $r$。這是一眼判斷哪個位點在起作用的摘要：$r$ 接近 $+1$ 的兩個系列取向相依性相同，也就是說這組資料無法區分這兩個位點。

### 基組診斷

診斷框的開頭幾列給出基組的狀態，每項一列。

```text
basis 347 (184 + 163)   F(s) ≤ 6.20 Å⁻¹   expanded-basis 6.7e-3
⚠ 擬合適用性未評估（v1 不保證定量占有率擬合）
⚠ Experimental：僅與 multislice 程式對比驗證（beta-AlCo [001], 250 keV）
```
