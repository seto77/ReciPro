...
Questa scheda tratta tre situazioni, a seconda di come la cella del sottogruppo si rapporta a quella del genitore:

- **Stessa cella convenzionale** — sottogruppi *translationengleiche* (*t*-) e sottogruppi *klassengleiche* (*k*-) che rimuovono soltanto vettori di centraggio (determinante della base del sottoreticolo $= 1$). Qui gli elementi di $H$ vivono nella cella del genitore, cosicché la sovrapposizione è una figura di una sola cella. (Una relazione *k*- che rimuove il centraggio, ad es. centrato $I$ o $F$ $\to$ primitivo, fa diventare rossi gli assi elicoidali e i piani di scorrimento generati dal centraggio.)
- **Ingrandimento di cella nel piano** — relazioni *k*- o isomorfe il cui ingrandimento giace nel piano *a*–*b* ($a'=n_a a$, $b'=n_b b$, con *c* invariato) in un genitore ortorombico, tetragonale o cubico. La cella ingrandita è disegnata (proiettata ⟂ *c*) come una griglia $n_a\times n_b$ di tessere della cella del genitore, e **ogni tessera è colorata in modo indipendente**: un elemento di simmetria conservato in $H$ appare in nero nelle tessere in cui quella copia sopravvive, e in rosso dove l'ingrandimento della cella l'ha rimosso — così un raddoppio di cella si vede direttamente come una copia su due che diventa rossa.
