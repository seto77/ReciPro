...
- **Temari** : pas un ajustement du tout, mais une table calculée à partir des premiers principes (calcul de champ auto-cohérent de Dirac complet avec échange KLI exact ; Temari dataset-factors v2.0.0, CC-BY-4.0). Comme une somme de gaussiennes décroît en $e^{-b s^2}$ alors que le vrai $f_e$ décroît en $s^{-2}$, les formes ajustées sortent de leur domaine au-delà de $s \approx 3$ Å⁻¹, ce qui n'arrive pas à une table calculée. **Atomes neutres Z = 1–86 uniquement** — les ions n'en sont pas dérivables — et, comme les ajustements ci-dessus, elle ne porte pas le $\gamma$ relativiste.
