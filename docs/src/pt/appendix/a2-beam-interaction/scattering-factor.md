...
O ReciPro oferece quatro fontes para $f_e(s)$: três parametrizações ajustadas e uma tabela calculada:
...
- **Temari** : não é um ajuste, mas uma tabela calculada a partir de primeiros princípios (cálculo de campo autoconsistente de Dirac completo com troca KLI exata; Temari dataset-factors v1.0.0, CC-BY-4.0). Como uma soma de gaussianas decai como $e^{-b s^2}$ enquanto o $f_e$ verdadeiro cai como $s^{-2}$, as formas ajustadas ficam fora de sua faixa além de $s \approx 3$ Å⁻¹, o que não acontece com uma tabela calculada. **Apenas átomos neutros Z = 1–86** — os íons não são deriváveis dela — e, como os ajustes acima, não carrega o $\gamma$ relativístico.
- **8-Gaussians** : um ajuste de oito termos válido sobre uma faixa mais ampla de $s$.

**Escolhendo um.** Os três ajustes visam o mesmo $f_e(s)$ subjacente e concordam estreitamente em $s$ baixo; eles diferem principalmente na faixa e em como o caroço atômico é representado. **Peng** (átomos neutros e íons comuns; as tabelas são ajustadas até $s \le 6$ Å⁻¹, mas como as caudas gaussianas caem mais rápido que a curva real, o ajuste só é efetivamente preciso até $s \lesssim 2$ Å⁻¹) é o padrão usual para fatores de estrutura de SAED/CBED; **Kirkland** estende-se a $s$ mais altos com um termo de caroço Lorentziano, adequado para HRTEM/STEM (lembre-se de $q=2s$); **8-Gaussians** é para reflexões que alcançam $s$ muito elevados. Para um elemento leve, os três são quase indistinguíveis; as diferenças aparecem para elementos pesados em ângulos altos. **Temari** é de outra natureza: não é ajustado a nada, portanto é a referência com que comparar um ajuste quando se quer saber o quanto ele se desviou — ao custo de cobrir apenas átomos neutros.
