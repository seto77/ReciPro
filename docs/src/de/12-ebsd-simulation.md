...
- **Kontrast fällt mit Energieverlust** (Standard an, E_c = 0.8 keV) : skaliert den Kikuchi-Kontrast jeder Energiescheibe mit A(E) = exp(−(E0 − E)/E_c), wobei E0 die Strahlenergie und E die Austrittsenergie ist, und verschiebt den verlorenen Anteil in einen flachen, richtungsgemittelten Sockel, sodass die Helligkeit ohne Bandkontrast unverändert bleibt. Die Monte-Carlo-Gewichte zählen jedes rückgestreute Elektron, doch Elektronen mit großem Energieverlust tragen kaum scharfen Kikuchi-Kontrast; ohne diese Korrektur trägt jede Scheibe vollen Kontrast, und die Bänder werden etwa 12 % breiter als für monochromatische Elektronen bei E0 (Si, 20 kV). Der Standardwert 0.8 keV wurde durch Anpassen der Bandbreiten an ein gemessenes Si-Muster bei 20 kV kalibriert; ob er für andere Materialien und Spannungen gilt, ist nicht bekannt. Eine Änderung gewichtet nur die vorhandenen Scheiben neu, ohne Master-Muster oder Monte-Carlo-Rechnung zu wiederholen.
