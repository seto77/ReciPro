...
Diese Registerkarte behandelt drei Situationen, je nachdem, wie die Zelle der Untergruppe mit der der Elterngruppe zusammenhängt:

- **Gleiche konventionelle Zelle** — *translationengleiche* (*t*-) Untergruppen sowie *klassengleiche* (*k*-) Untergruppen, die nur Zentrierungsvektoren entfernen (Determinante der Untergitter-Basis $= 1$). Hier liegen die Elemente von $H$ in der Zelle der Elterngruppe, sodass die Überlagerung ein einzelnes Zellbild ist. (Eine zentrierungsentfernende *k*-Beziehung, z. B. $I$- oder $F$-zentriert $\to$ primitiv, lässt die von der Zentrierung erzeugten Schraubenachsen und Gleitspiegelebenen rot werden.)
- **Zellvergrößerung in der Ebene** — *k*- oder isomorphe Beziehungen, deren Vergrößerung in der *a*–*b*-Ebene liegt ($a'=n_a a$, $b'=n_b b$, bei unverändertem *c*) in einer orthorhombischen, tetragonalen oder kubischen Elterngruppe. Die vergrößerte Zelle wird (⟂ *c* projiziert) als $n_a\times n_b$-Gitter aus Zellkacheln der Elterngruppe gezeichnet, und **jede Kachel wird unabhängig eingefärbt**: ein in $H$ erhaltenes Symmetrieelement erscheint schwarz in den Kacheln, in denen diese Kopie überlebt, und rot dort, wo die Zellvergrößerung sie entfernt hat — so zeigt sich eine Zellverdopplung direkt als jede zweite rot werdende Kopie.
