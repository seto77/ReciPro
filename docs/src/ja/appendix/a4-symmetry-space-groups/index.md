...
!!! note "Group Relations... は現在も開発が進行中の機能です"
    **Group Relations...**（群の関係）ブラウザ（A4.2）は、*translationengleiche*（t-）部分群/超群と *klassengleiche*（k-）*部分群* を、あらかじめ用意された表からではなく空間群自身の対称操作から直接計算します。そのため表示される各関係は、表からの転記ではなく独立に検証可能です。一部の枝——k-*超群* と *isomorphic*（同型）部分群——は現バージョンではまだプレースホルダです。どのタブ・どの枝が現在稼働しているかは A4.2 で正確に述べます。
