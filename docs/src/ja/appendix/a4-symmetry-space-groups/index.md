...
!!! note "Group Relations... は現在も開発が進行中の機能です"
    **Group Relations...**（群の関係）ブラウザ（A4.2）は、*translationengleiche*（t-）と *klassengleiche*（k-、*isomorphic*（同型）を含む）の部分群・超群を、あらかじめ用意された表からではなく空間群自身の対称操作から直接計算します。そのため表示される各関係は、表からの転記ではなく独立に検証可能です。残る制限——たとえば同型系列の列挙は index ≤ 4 まで——は A4.2 の「現在の制限」に明記されています。
