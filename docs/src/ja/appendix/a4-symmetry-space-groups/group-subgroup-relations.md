...
- **ツリー**（左ペイン）には **極大部分群** と **極小超群** の2つの根があり、それぞれが **`t — translationengleiche`** の枝、**`k — klassengleiche`** の枝、そして（極大部分群の下にのみ）**`同型 (系列)`** の枝に分かれます。同じ子タイプ・同じ指数を持つ非共役な類はそのままではラベルが同一になるため、`· 類 n` の接尾辞で区別されます。
- **Diagram**（系統図）タブは、簡略化したBärnighausen図式風の骨格を描画します。中央に現在の群（強調表示）、その上に最大6個までの超群、下にすべての極大 *t*-部分群を配置し、指数を記したラベル付きの線（例: `t2`, `t4`）で結びます。ノードの記号は、らせん軸の下付き数字や回反のオーバーラインを含む正式な結晶学記号として組版されます。これが表すのはあくまで群論的な骨格のみです——構造的関係としての本来のBärnighausen図では、胞の変換・Wyckoff分裂・原子座標の対応関係も各辺に付記されますが、それらは図そのものではなく後述の各タブに収められています。**このグラフに描かれるのは *t*-関係のみ** です。*k*-関係はツリーには表示されますが、ここには現れません。
...
- ***k*-超群** は初回利用時にバックグラウンドで計算されます（逆引きには同じ結晶類に属する全タイプの *k*-部分群表が必要なため）。準備が整うまで、ツリーには一時的にグレーの「計算中…」ノードが表示されます。
