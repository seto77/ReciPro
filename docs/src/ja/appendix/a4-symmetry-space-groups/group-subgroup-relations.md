...
- **ツリー**（左ペイン）には **極大部分群** と **極小超群** の2つの根があり、それぞれが **`t — translationengleiche`** の枝、**`k — klassengleiche`** の枝、そして（極大部分群の下にのみ）**`同型 (系列)`** の枝に分かれます。
- **Diagram**（系統図）タブは、簡略化したBärnighausen図式風の骨格を描画します。中央に現在の群（強調表示）、その上に最大6個までの超群、下にすべての極大 *t*-部分群を配置し、指数を記したラベル付きの線（例: `t2`, `t4`）で結びます。これが表すのはあくまで群論的な骨格のみです——構造的関係としての本来のBärnighausen図では、胞の変換・Wyckoff分裂・原子座標の対応関係も各辺に付記されますが、それらは図そのものではなく後述の各タブに収められています。**このグラフに描かれるのは *t*-関係のみ** です。*k*-関係はツリーには表示されますが、ここには現れません。
...
***t*-部分群** の場合に限っては、並進格子が変化しない（$T_G=T_H$）ため、群論的にはここで **反位相（並進）ドメイン** というものは存在しません——すべてのドメイン状態は、単なるシフトではなく、必ず真の点群操作によって基準状態と異なります。そのためこのタブは常に `antiphase = 1`、`orientation = total` と報告します。すなわち $[G:H]$ 個のドメイン状態すべてが **方位ドメイン** です。（反位相・並進ドメインが可能になるのは *k*-部分群への転移——実際に並進が失われる場合——のみです。「現在の制限」を参照。）
...
現バージョンでは *t*-関係でのみ利用できます（後述の「現在の制限」参照）。消失した操作が無い場合（指数1は表示されませんが、コセット代表元のリストが空になる退化したケース）は「(単一ドメイン)」と報告されます。
...
*t*-部分群は単位胞を拡大しないため、これらは **超構造反射・分数指数反射ではありません**——親の胞のままの整数 $(h,k,l)$ であり、それをこれまで消滅させていた映進面やらせん軸が失われたために「許容」になるだけです。（親胞に対して分数指数を持つ真の超構造反射は、胞そのものが拡大して初めて生じ得るものであり、それは *t*-部分群ではなく *k*-部分群で起こります。一般に *k*-部分群は、より大きな胞に由来する分数指数反射と、親の心付け並進の一部が失われることによる整数指数反射——この節で述べた *t*-専用の場合と同様の性質のもの——の両方を同時に解除しえますが、このタブはまだ *k*-関係についてはどちらの種類も計算しません。後述参照。）ここに表示される反射は対称性上 *許容* されるにすぎず、実際に観測されるかどうかは、実際の低対称構造の構造因子に依存します。
...
ブラウザの *t*-部分群・*t*-超群エンジン、および *k*-部分群エンジンは完全に実装済みであり、空間群の操作表と照合して独立に検証されています。一部の枝はまだプレースホルダであり、黙って省略するのではなくその旨が明示されます。

- **同型部分群**（専用のツリーの枝）と ***k*-超群**（極小超群の下の *k* の枝）はまだ計算されておらず、ツリーにはどちらもグレーアウトしたプレースホルダのノードが表示されます。
- **軌道分裂**、**ドメイン・双晶**、**新規反射** は *t*-関係でのみ実装されています。*k*-関係を選択すると、3つのタブすべてで表の代わりに説明メッセージが表示されます。これは、並進格子が変化しないことを前提に書かれたサンプル点／mod 1 のロジックを、より粗い並進格子を持つ *k*-関係にそのまま流用すると誤った結果を返してしまうためです。専用の *k*- 用実装（胞が拡大して初めて意味を持つ「分数次／解除／保持」の3分類による反射分類を含む）は、今後のPhaseで対応予定です。
- **系統図 (Diagram)** タブは *t*-関係のみを描画します。*k*-関係はツリー上には見えますが、まだグラフとしての表現はありません。
