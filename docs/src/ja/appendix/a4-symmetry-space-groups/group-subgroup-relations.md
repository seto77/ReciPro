...
このタブは ***translationengleiche* (*t*-) 部分群でのみ**提供されます。*t*- では $H$ が親の並進格子を保つ（$T_H=T_G$）ため、両群の要素が同じ胞内に収まります。*k*-・同型関係では胞が拡大するため図は将来版に見送り、タブには **Domains & Twins** / **New reflections** タブ（それらのケースでは既に失われる格子対称の情報を持つ）を案内する短い注記を表示します。
