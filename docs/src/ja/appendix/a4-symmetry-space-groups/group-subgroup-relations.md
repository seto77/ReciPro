...
**変換行列 (Matrix)** タブが保持・消失する操作を表で示すのに対し、このタブは同じ情報を*幾何的に*——親の[対称要素図](symbols-and-diagrams.md#symmetry-element-diagram)（軸・面・反転中心を描く ITA Vol.A 風の図）に重ねて——示します。「結晶が $G$ から部分群 $H$ へ転移するとき、どの対称要素が生き残り、どれが壊れるか」を、結晶学者が既に読み慣れた 1 枚の図で一目で示します。

- **$H$ で保持される要素は黒、失われる要素は赤**で描かれます。投影方向（⟂ *a*, *b*, *c*）は親の結晶系に応じて自動選択され、ヘッダに関係と投影方向を表示します。
- 対称要素がより低次のものへ退化する場合はそのまま図示されます。例えば 4 回軸が 2 回軸になる場合、**赤い 4 回記号の上に黒い 2 回記号**が重なって描かれます——赤が「4 回軸は失われた」、黒が「ここに 2 回軸が残る」を意味します。
- 重ね描きは、親の完全な図を（*lost* の下地として）描いた上に、$H$ 自身の操作集合から直接再構築した対称要素を上書きして作ります——したがって保持される要素は $H$ に現れるものそのものであり、記号ごとの当て推量はありません。

このタブは、部分群の胞が親の胞とどう関係するかに応じて 3 つのケースを扱います。

- **同一の慣用胞** — *translationengleiche*（*t*-）部分群、および心付けベクトルを除去するだけの *klassengleiche*（*k*-）部分群（部分格子基底の行列式 $\det = 1$）です。$H$ の要素が親の胞内に収まるため、重ね描きは親胞 1 個の図になります。（例えば $I$ 心・$F$ 心 $\to$ 単純格子のような心付け除去 *k*- 関係では、心付けが生む映進面やらせん軸が赤くなります。）
- **面内での胞拡大** — 拡大が *a*–*b* 面内にある（$a'=n_a a$, $b'=n_b b$ で *c* は不変）直方晶・正方晶・立方晶の親における *k*-・同型関係です。拡大胞を（⟂ *c* に投影して）親胞タイル $n_a\times n_b$ 個の格子として描き、**各タイルを個別に着色**します。$H$ で保持される対称要素は、そのコピーが残るタイルでは黒、胞拡大によって除かれたタイルでは赤になります——胞の倍化が「一つおきのコピーが赤くなる」形でそのまま可視化されます。
...
![対称要素タブ](../../../assets/cap-ja-auto/FormGroupRelations-tabElements.png)
