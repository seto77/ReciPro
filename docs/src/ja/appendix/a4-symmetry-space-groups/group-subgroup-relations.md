...
- **ツリー**（左ペイン）には **極大部分群** と **極小超群** の2つの根があり、それぞれが **`t — translationengleiche`** の枝、**`k — klassengleiche`** の枝、そして **`同型 (系列)`** の枝に分かれます。同じ子タイプ・同じ指数を持つ非共役な類はそのままではラベルが同一になるため、`· 類 n` の接尾辞で区別されます。極大部分群側の **同型** 枝では、$G$ の *affine normalizer* のもとで同値な共役類がさらに 1 つの軌道行（「… — 同値な m 類 (normalizer)」）にまとめられます——ITA Vol. A1 の IIc エントリと同じ粒度です。列挙の上限はツールバーの **同型部分群: index ≤** スピナー（2〜27、既定 4。上限を上げた分はバックグラウンドで計算）で設定します。
- **系統図 (Diagram)** タブは、簡略化したBärnighausen図式風の骨格を描画します。中央に現在の群（強調表示）、その上に極小超群、下に極大部分群を配置します——***t*-・*k*-・同型のいずれの関係も**、それぞれが「極大な一段」なので同じ段に並びます。各辺には種別と指数のラベル（`t2`, `k3`, `i3`）が付き、*t*=青、*k*=青緑、同型=橙で色分けされます。ノードの記号は、らせん軸の下付き数字や回反のオーバーラインを含む正式な結晶学記号として組版されます。同じ対象タイプ・種別・指数を持つ非共役な類は1つのノードに集約され、辺ラベルに類数が添えられます（例: `k2 ·2類`）——類ごとの詳細はツリー側で確認できます。1行に収まらない数の関係がある場合はノードが一段縮小され、なお収まらない分は破線の `+N` ノードにまとめられます（クリック不可——全リストはツリーで参照）。同型の辺が表示されているときは隅に `i: index ≤ 4 のみ` の注意書きが、*k*-超群の逆引きを構築中は `k: 計算中…` が表示されます。ダブルクリックで部分群へ降りていくと、通ってきた群の連なり（*選択パス*）が現在の群の上に紫の縦列として描かれます——自分がたどった転移経路の多階層Bärnighausen図です（例: $Pm\bar3m \rightarrow P4/mmm \rightarrow Pmmm \rightarrow \ldots$）。各辺には経由した関係のラベルが付き、上へ昇る操作や **戻る** ボタンでパスは相応に切り詰められます。祖先が3つを超える分はグレーの `⋮ +N` に省略されます。これが表すのはあくまで群論的な骨格のみです——構造的関係としての本来のBärnighausen図では、胞の変換・Wyckoff分裂・原子座標の対応関係も各辺に付記されますが、それらは図そのものではなく後述の各タブに収められています。
...
*t*-部分群への転移について、$G$ では系統的に消滅していたが $H$ では対称性上許容されるようになる反射を一覧表示します——すなわち、親の反射条件（[反射条件](../../2-symmetry-information.md) 参照）では禁止されているが、$H$ の反射条件では禁止されない反射です。探索範囲はタブ上の **探索範囲** スピナーで設定します: 既定は $|h|,|k|,|l|\le4$ で、2〜8 の範囲で調整できます（上限を上げると行数が大幅に増えることがあります）。
...
***k*- および同型の関係** では、このタブは新規反射を **拡大した部分群胞の指数で**（同じく探索範囲内で）一覧表示し、右端の列で次の2種類に分類します。
...
ブラウザの他の部分が「空間群」の間を移動するのに対し、このタブはその移動の背景となる全体地図——**32 の結晶学的点群型（幾何結晶類）のハッセ図**、すなわち「どの型がどの型の部分群として現れるか」の半順序——を表示します。各ノードは 1 つの点群型で、縦軸は群の位数（下端が 1、上端が 48、対数目盛）。左のタワーが六方・三方晶族、右のタワーが立方–正方–斜方系です。辺は *極大*（被覆）部分群関係——間に第 3 の型が厳密に挟まらない関係——を表し、32 型の間にちょうど 80 本あります。この図は半順序ですが、数学的な意味での束 (lattice) ではありません: 互いに比較できない 2 つの最大元 $m\bar3m$ と $6/mmm$ を持ちます。

- 表示中の空間群の点群には（系統図タブの現在ノードと同じ）水色のハローが付き、既定ではこれが「注視」型です。
- 任意のノードを**クリック**するとその型を注視します: その**部分群型は青**、**超群型は橙**で強調され、注視ノード自身の辺に表示される数字はその極大ステップの **index（位数比）** です。注視と無関係な型はグレーのままです。余白をクリックすると現在の点群に戻ります。（クリックしてもどこへも移動しません——1 つの点群型は 1 つではなく多数の空間群に対応するためです。）
- 上部のキャプションは注視型の要約——Hermann–Mauguin 記号と Schoenflies 記号、位数 $|G|$、230 空間群型のうち何型がこの点群に属するか、部分群型・超群型の数——を示します。

この図は Hermann の定理の点群への射影です: ツリーでの *t*-部分群 1 ステップは、この図で辺をちょうど 1 本下ることに対応し（*t*-ステップの空間群 index は辺に示される点群 index に等しい）、*k*-・同型ステップは同じノードに留まります。

![点群タブ](../../../assets/cap-ja-auto/FormGroupRelations-tabPointGroups.png)

---

## 対称要素・一般位置タブ (Elements & Positions)

**変換行列 (Matrix)** タブが保持・消失する操作を表で示すのに対し、このタブは同じ情報を*幾何的に*——結晶学者が ITA Vol.A で読み慣れた 2 枚の図を左右に並べて——示します。左は親の[対称要素図](symbols-and-diagrams.md#symmetry-element-diagram)（軸・面・反転中心）、右は親の**一般位置図**です。両図とも 1 つの配色規則——**$H$ で生き残るものは黒、失われるものは黄色**（単位胞は下地としてブルー）——に従うため、「結晶が $G$ から部分群 $H$ へ転移するとき、どの対称要素が壊れ、一般位置の軌道がどう分裂するか」が一目で分かります。投影方向（⟂ *a*, *b*, *c*）は親の結晶系に応じて自動選択され、ヘッダに関係・種別と指数・投影方向を表示します。

**左 — 対称要素図**

- 重ね描きは、失われる親の要素を黄色で描いた上に、$H$ 自身の操作集合から直接再構築した対称要素を黒で上書きして作ります——したがって保持される要素は $H$ に現れるものそのものであり、記号ごとの当て推量はありません。
- 対称要素がより低次のものへ退化する場合は、残る側の記号だけが描かれます。例えば 4 回軸が 2 回軸だけを残す位置には**黒い 2 回記号のみ**が現れます。失われた要素が黄色で描かれるのは、その幾何的位置に何も残らない場合だけです——残った対称の読み取りやすさを優先します。

左図は、部分群の胞が親の胞とどう関係するかに応じて 3 つのケースを扱います。

- **同一の慣用胞** — *translationengleiche*（*t*-）部分群、および心付けベクトルを除去するだけの *klassengleiche*（*k*-）部分群（部分格子基底の行列式 $\det = 1$）です。$H$ の要素が親の胞内に収まるため、重ね描きは親胞 1 個の図になります。（例えば $I$ 心・$F$ 心 $\to$ 単純格子のような心付け除去 *k*- 関係では、心付けが生む映進面やらせん軸が黄色くなります。）
- **面内での胞拡大** — 拡大が *a*–*b* 面内にある（$a'=n_a a$, $b'=n_b b$ で *c* は不変）直方晶・正方晶・立方晶の親における *k*-・同型関係です。拡大胞を（⟂ *c* に投影して）親胞タイル $n_a\times n_b$ 個の格子として描き、**各タイルを個別に着色**します。$H$ で保持される対称要素は、そのコピーが残るタイルでは黒、胞拡大によって除かれたタイルでは黄色になります——胞の倍化が「一つおきのコピーが黄色くなる」形でそのまま可視化されます。
- **それ以外**（視線軸 *c* 方向への拡大、斜交・非直交な胞変換、六方晶・三方晶・単斜晶の親）では、失われる対称を 2 次元投影で曖昧さなく描けないため、タブには **Domains & Twins** / **New reflections** タブ（それらのケースでは既に失われる格子対称の情報を持つ）を案内する短い注記を表示します。

**右 — 一般位置図**：$G$ のもとでは描かれた全ての点が 1 つの軌道（1 つの一般位置）を成しますが、$H$ のもとではそれが $[G:H]$ 個の副軌道に分裂します（**軌道分裂 (Orbit splitting)** タブと対応）。代表点を含む副軌道——$H$ の 1 つの一般位置として残る組——は**黒**、対称性の低下により代表点と等価でなくなった残りの副軌道の点は**黄色**で描かれます。投影によって等価な 2 点が重なる位置には ITA 標準の *split circle*（縦線で二分した円。鏡像側の半分にコンマ、各半分の脇に高さラベル `+`, `½−` など）を使います。**片方の半分だけが $H$ で生き残る場合は、縦の分割線と、失われた側のコンマ・高さラベルが黄色**で描かれるため、「半分だけ失われた」位置も図から直接読み取れます。軌道の色分けは同一胞の関係（*t*- および心付け除去 *k*-）で表示され、胞拡大を伴う関係では右パネルに短い注記が出ます。

菱面体軸 (Rho) 設定で表示中の R 空間群では、どちらの図もこの投影では描けないため、タブには注記が表示されます。両図は同じ R 空間群の六方軸 (Hex) 設定で表示できます。

![対称要素・一般位置タブ](../../../assets/cap-ja-auto/FormGroupRelations-tabElements.png)
...
- **同型部分群の列挙はスピナーの上限まで（既定 index ≤ 4、最大 27）** です。同型系列はより高い指数へ際限なく続くため、枝のグレーの注記には常に現在の上限が明示され、一覧が完全であるかのようには表示されません。normalizer 軌道へのまとめは normalizer 生成元の有界探索に基づいており、検証済みのケースでは ITA A1 と一致しますが、全空間群に対する完全性の形式的証明は今後の課題です——最悪の場合でも 1 つの軌道が複数行に分かれて表示されるだけで、誤って併合されることはありません。
