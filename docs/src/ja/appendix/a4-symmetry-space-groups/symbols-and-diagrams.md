...
ヘルマン・モーガン記号には2つの階層があります。**点群記号**（上段のボックス、*Point Group*）は結晶の巨視的対称性のみを表し、**空間群記号**（下段のボックス、*Space Group*）はこれに格子の心付け (centering) と、らせん・映進成分を加えたものです。
...
**Sohncke 群 (向きを保つ群)** — *すべて*の操作の線形部が $\det R=+1$ を満たすこと。すなわち群は真の回転・らせん回転のみを含み、鏡映・映進・反転・回反を一切含みません。230タイプ中65タイプがSohncke群です。Sohncke群であることは、構造がその鏡像を含むことなく特定の掌性を持つ物体（キラル分子・タンパク質・水晶、…）と両立するための対称性条件です。これは、230タイプの中で自身の鏡像となる別タイプを持つ本当の意味での「対」（下記「掌性対の相手」参照）のメンバーであることよりも広い概念です。
...
## 対称要素の模式図 {#symmetry-element-diagram}

![対称要素・一般位置の模式図](../../../assets/cap-ja-auto/FormSymmetryInformation.splitContainer1.tableLayoutPanel1.png)

左側の模式図は、**投影方向**（`a`/`b`/`c`）で選んだ投影軸に沿って見た、現在の設定に対するITA Vol. A様式の対称性模式図を再現します。
...
!!! note "F格子の立方晶群: 単位胞の一部のみを描画"
    立方晶系の $F$心付け空間群では、ReciProは投影した単位胞の左上1/4のみを描画します（そうしないと図が密集しすぎて読めなくなるためです）。残りの領域は、この心付け並進と描画済みの対称要素自体によって繰り返されます。同じ対称要素は [Structure Viewer](../../5-structure-viewer.md) の3Dモデル上に直接重ねて表示することもできます。
...
**投影方向** の下にある $x$ / $y$ / $z$ の欄では、試験点 $(x,y,z)$ をその点群の既定位置から動かすことができます。複数の円が重なって図が見づらい場合に有用です。
