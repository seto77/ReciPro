...
- **系統的(空間群による)消滅.** 格子の心付け(centering: 体心・面心・底心など)や、並進成分を持つ対称要素(らせん軸・映進面)は、原子の中身によらずその空間群のすべての結晶で、ある反射群を *厳密に* 消します。これらが **消滅する面は非表示** の根拠です。
- **偶然的な準消滅.** ある特定の構造で原子寄与がたまたま打ち消し合うと、強度は小さいが対称性で禁制ではなく、組成や位置が変われば再び現れます。これらは消滅則では除外されません。

系統消滅は、単位胞の対称関連コピー間で位相が打ち消し合う現象です。心の並進(体心・面心・底心などの centering 並進)$\mathbf t_\alpha$ に対し、構造因子は共通因子
