...
X線の **光電吸収** が内殻電子を叩き出す([減衰・輸送](attenuation-transport.md)参照)と、深い準位に空孔が残ります。原子は外殻の電子を空孔に落として緩和し、放出されるエネルギーは **特性X線光子**(蛍光)として出るか、もう1つの電子を叩き出す(**Auger** 過程)かのいずれかになります。**蛍光** タブは特性光子チャネルをプレビューします。これは X線専用で、電子線・中性子線では非表示です。
...
スペクトルが連続ではなく数本の離散線(K$\alpha_1$, K$\alpha_2$, K$\beta_1$, L$\alpha_1$, …)になるのは、殻のエネルギー自体が量子化されているためです。そのうちどの遷移が強く出るかを決めるのが電気双極子選択則です(禁制遷移・多重極遷移・サテライト線も弱いながら存在します)。そのエネルギーは **Moseley 則** に従います。遮蔽水素様近似では、
...
ここで $\sigma$ は遮蔽定数です。$K\alpha$($n_2{=}2\to n_1{=}1$, $\sigma\approx1$)ではこれが $E_{K\alpha}\approx R_\infty hc\,(Z-1)^2\left(1-\tfrac14\right)$ になります。この $Z$ 依存性を決めているのは **核電荷** であり、残りの電子はそれを遮蔽するだけです。したがって単調で、化学状態にもほとんど影響されません。これが元素同定(EDX/WDX)の基礎です。
