...
- **Temari** : フィットではなく、第一原理計算による表です(完全 Dirac 自己無撞着場計算 + 厳密交換の KLI 近似。Temari dataset-factors v2.0.0、CC-BY-4.0)。ガウシアンの和は $e^{-b s^2}$ で減衰するのに対し真の $f_e$ は $s^{-2}$ で落ちるため、$s \approx 3$ Å⁻¹ を超えるとフィット形は適用範囲外になりますが、計算表はそうなりません。**中性原子 Z = 1–86 のみ**(イオンはこの表からは導出できません)。上のフィット同様、相対論因子 $\gamma$ は含みません。
