...
Master pattern は、検出器画像そのものではなく、結晶に固定された方向空間上の強度分布です。ReciPro は方向空間を Roşca–Lambert 等積投影でサンプリングし、$+Z$ 半球と $-Z$ 半球を別々の plane 配列として保持します。等積投影を使うことで、極付近と赤道付近のサンプリング密度の偏りを抑えられます。
...
相反定理により、方向 $\widehat{\mathbf s}$ の master pattern は $-\widehat{\mathbf s}$ 方向から入射する平面波の問題として、方向ごとに結晶表面を $\widehat{\mathbf s}$ に垂直とみなして解かれます。そのため厚さの座標 $t$ は出射方向に沿って測った長さです。一方、Monte Carlo が与える源の深さ $z$ は、傾斜した試料表面の法線方向に測った深さです。出射方向と表面法線のなす角を $\chi$、$\mu=\cos\chi$ とすると、深さ $z$ の源から表面までの出射方向に沿った経路長は

$$t=\frac{z}{\mu}$$

です。ReciPro は、出射方向のビンとエネルギーごとに深さ分布を指数分布 $\propto e^{-z/\lambda_z(E)}$ で当てはめ、検出器の画素ごとにその画素の $\mu$ で $\lambda_t=\lambda_z/\mu$ に換算し、深さ配列の範囲で規格化した分布を master pattern の厚さ区間ごとに積分して重みとします。したがって上の和の深さ添字は厚さ $t_j$ を走り、画素の重みは $\widehat{\mathbf s}$ だけでなくその画素の $\mu$ にも依存します。表面すれすれの出射（$\mu$ が小さい）ほど、同じ深さの源でも長い経路を通るので、重みは厚いスライスへ移ります。

自動設定される深さ配列は、検出器に届く電子の経路長 $z/\mu$ の 99.9 パーセンタイル（有効数字 2 桁に切り上げ）を上限とし、表面近くほど細かい 40 点の不等間隔です。
