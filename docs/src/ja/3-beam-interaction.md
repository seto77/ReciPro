...
- **Temari** はフィットではなく計算による表です（Temari dataset-factors v2.0.0、CC-BY-4.0。完全 Dirac 自己無撞着場計算による）。対応は **中性原子 Z = 1–86 のみ** で、イオンと Z = 87–98 には値がありません。中性原子の値で代用せず、曲線を描かず表のセルを空欄にします。
