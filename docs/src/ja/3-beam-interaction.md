...
|------|------|------|------|------|
| **X線** | 異常分散込みの構造因子 | µ/ρ・µ・透過率 ＋ 吸収端（エネルギー依存） | $f(s)$（Waasmaier–Kirfel / Temari）または $F(q)+S(q)$ | 特性線 ＋ EDX スティック |
| **電子線** | 電子構造因子 | σ・MFP・\|dE/ds\|・IMFP・飛程（エネルギー依存） | Peng / Kirkland / 8-Gaussians / Temari | —（非表示） |
...
- 横軸セレクタで **2θ**（散乱角・度）、**d**（格子面間隔）、**Q**（$= 4\pi\sin\theta/\lambda$、散乱ベクトル＝運動量移行）を選べます。3 つは同じ反射群を表し、横軸の取り方だけが変わります。
...
縦線は現在のエネルギーと、各元素の **吸収端** を示します。左のスカラ表には、現在のエネルギーでの **µ/ρ（全質量減衰係数）**・**µ（線減衰係数）**・**減衰長**（$1/\mu$）・**HVL**（半価層、$\ln 2/\mu$）・厚さ *t* での **透過率**・**µ_en/ρ**（質量エネルギー吸収係数）、X線屈折率の減少量 **δ** と **β**（$n = 1-\delta+i\beta$）、全反射の **θc (臨界角)**、実部の **X線 SLD**（散乱長密度）が並びます。下の表は各元素の **K**・**L3** **吸収端** のエネルギーと **ジャンプ比** です。
...
- **σ 弾性 (nm²)** — 弾性散乱断面積: 原子1個が電子をどれだけ曲げやすいか。
- **弾性MFP (nm)** — 弾性平均自由行程: 弾性散乱から次の弾性散乱までに電子が進む平均距離。
- **|dE/ds| (keV/nm)** — 阻止能の大きさ: 電子が1nm進むごとに失うエネルギー。
...
- **X線** には2つの **モデル** モードがあります。**f(s)** は慣用の X線原子散乱因子（電子単位）を、**F(q)+S(q)** は Rayleigh **コヒーレント**形状因子 $F(q)$ と Compton **非コヒーレント**散乱関数 $S(q)$（xraylib）を描きます。表には現在のエネルギーでの異常分散項 **f'(E)**・**f''(E)** に加え、f(s) 列がどの出典から来たかを示す **モデル** 列も載ります。**f(s)** を選ぶと、曲線を **どこから取るか** を選ぶラジオボタンの行がもう 1 つ現れます — **Waasmaier–Kirfel**（既定のフィット。イオンを含む Z = 1–98 に対応）または **Temari**。**F(q)+S(q)** の表示中は、その 2 曲線が xraylib 固有で出典を選べないため淡色になります。
- **電子線** には電子散乱因子の4つの出典があります: **Peng**・**Kirkland**・**8-Gaussians**・**Temari**。表は $f_e(s)$（nm）と、それを与えた **モデル** を示します。
- **中性子線** の散乱長は $s$ に依存しないため曲線は描かれず、各元素のコヒーレント散乱長 **b_coh** とコヒーレント / 非コヒーレント断面積を表に示します。
- **Temari** はフィットではなく計算による表です（Temari dataset-factors v1.0.0、CC-BY-4.0。完全 Dirac 自己無撞着場計算による）。対応は **中性原子 Z = 1–86 のみ** で、イオンと Z = 87–98 には値がありません。中性原子の値で代用せず、曲線を描かず表のセルを空欄にします。
