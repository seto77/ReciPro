...
### 症状: Windows Defender（やアンチウイルス）がインストーラを検知する

**原因**: **誤検知** です。本ソフトは複数のウイルススキャナで確認済みでクリーンです。Windows Defenderが圧縮された `.msi` インストーラに対して誤った挙動をしていました。(Issue [#32](https://github.com/seto77/ReciPro/issues/32))

**解決方法**: v4.881以降、これを避けるためインストーラは**非圧縮**で配布しています（そのためファイルサイズは大きくなっています）。最新のインストーラを使い、それでもアンチウイルスが警告する場合は許可して問題ありません。
...
はい。最近のバージョンは、.NET Desktop Runtime をインストールすればARM64版Windowsで動作します（v4.913 付近・.NET 10 で動作確認済み）。(Issue [#47](https://github.com/seto77/ReciPro/issues/47))
