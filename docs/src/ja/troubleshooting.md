...
**解決方法**: GPUドライバを最新版に更新してください。最低要件は OpenGL 1.3 で、OpenGL 4.3 対応の外付けGPUを推奨します（[動作環境](index.md) 参照）。
...
- メイン描画領域を**右クリック**して縮小する（この描画領域はホイールズームに対応していません。[21. キーボード・マウスショートカット](21-shortcuts.md) 参照）。
- 右側の **Wave source** パネルで入射波のエネルギーを確認する。X線は約 1–100 keV、電子線は約 10–1000 keV が適切です。
