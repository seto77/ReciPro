...
本选项卡根据子群晶胞与母群晶胞的关系，处理三种情形：

- **相同惯用胞** — *translationengleiche*（*t*-）子群，以及仅移除心付矢量的 *klassengleiche*（*k*-）子群（子点阵基的行列式 $= 1$）。此时 $H$ 的要素落在母群晶胞内，因此重叠图为单个晶胞的图。（移除心付的 *k*- 关系，例如 $I$ 心或 $F$ 心 $\to$ 简单格子，会使心付所生成的螺旋轴与滑移面变红。）
- **面内晶胞扩大** — 扩大位于 *a*–*b* 面内（$a'=n_a a$，$b'=n_b b$，*c* 不变）的正交、四方或立方母群的 *k*- 或同型关系。扩大后的晶胞（投影 ⟂ *c*）绘制为 $n_a\times n_b$ 个母胞瓦片的网格，且**每块瓦片独立着色**：在 $H$ 中保持的对称要素，在该副本得以存留的瓦片中为黑色，在晶胞扩大已将其移除处为红色 — 于是晶胞倍增直接表现为每隔一个副本变红。
