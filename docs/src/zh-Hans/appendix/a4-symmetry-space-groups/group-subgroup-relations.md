...
对 *t*-子群转变，列出在 $G$ 中系统消光、但在 $H$ 中变为对称性允许的反射（搜索范围 $|h|,|k|,|l|\le4$） — 即被母群的反射条件（见[衍射条件](../../2-symmetry-information.md)选项卡）禁止、而 $H$ 的反射条件并不禁止的反射。
...
对 ***k*- 或同型关系**，选项卡以**扩大后的子群胞指数**列出新反射（同样以 $|h'|,|k'|,|l'|\le4$ 为限），并在最后一列将每条反射归类：
...
- **同型子群仅枚举到 index ≤ 4。** 同型系列会无限延续到更高指数（对立方晶群，仅 $a'=3a$ 就已是指数 27），因此树在该分支上标注灰色的“仅显示 index ≤ 4 — 同型系列延伸至更高指数”注记，而不是佯装列表已经完整。
- ***k*-超群** 在首次使用时于后台计算（反查需要同一晶类中每种类型的 *k*-子群表）；就绪之前，树会短暂显示灰色的“计算中…”节点（系统图角落则显示“k: 计算中…”注记）。
- 反射搜索窗口固定为 $|h|,|k|,|l|\le4$，当前版本不可由用户调整。
