...
## 对称要素与一般位置选项卡（Elements & Positions）

**变换矩阵**选项卡以表格形式列出保持与消失的操作，而本选项卡则把同样的信息以*几何方式*展示出来，用的正是晶体学家在 ITA Vol. A 中早已习惯左右对照阅读的那两幅图：左边是母群的[对称元素示意图](symbols-and-diagrams.md#symmetry-element-diagram)（轴、面和反演中心），右边是母群的**一般位置图**。两幅图遵循同一条配色规则 — **在 $H$ 中保持的为黑色，消失的为黄色**，单位胞则以蓝色绘于底层 — 于是本选项卡让人一眼看清：当晶体从 $G$ 转变为子群 $H$ 时，哪些对称要素被破坏，一般位置的轨道又如何分裂。投影方向（⟂ *a*、*b* 或 *c*）根据母群的晶系自动选择，标题栏注明该关系、其种类与指数，以及投影方向。

**左 — 对称要素**

- 重叠图先以黄色画出母群中消失的要素，再在其上以黑色绘出直接由 $H$ 自身操作集重构出的对称要素 — 因此保持的要素恰好就是那些在 $H$ 中重新出现者，无需逐个符号地猜测。
- 对称要素退化为较低者时，只画出存留下来的那个符号：在四次轴只剩下其二次轴的位置，出现的仅是**黑色的二次轴符号**。消失的要素只有在其几何位置上一无所留时才以黄色画出，从而保证存留的对称清晰可读。

左图根据子群晶胞与母群晶胞的关系，处理三种情形：

- **相同的常规晶胞** — *translationengleiche*（*t*-）子群，以及仅移除带心矢量的 *klassengleiche*（*k*-）子群（子点阵基的行列式 $\det = 1$）。此时 $H$ 的要素落在母群晶胞内，因此重叠图为单个母胞的图。（移除带心的 *k*- 关系，例如 $I$ 心或 $F$ 心 $\to$ 简单点阵，会使带心所生成的螺旋轴与滑移面变为黄色。）
- **面内晶胞扩大** — 扩大位于 *a*–*b* 面内（$a'=n_a a$，$b'=n_b b$，*c* 不变）的正交、四方或立方母群的 *k*- 或同型关系。扩大后的晶胞（投影 ⟂ *c*）绘制为 $n_a\times n_b$ 个母胞瓦片的网格，且**每块瓦片独立着色**：在 $H$ 中保持的对称要素，在该副本得以存留的瓦片中为黑色，在晶胞扩大已将其移除处为黄色 — 于是晶胞倍增直接表现为每隔一个副本变黄。
- **其他情形**（沿视线轴 $c$ 的扩大、斜交/非正交的晶胞变换、六方/三方/单斜母群）失去的对称无法在二维投影中无歧义地绘制，因此选项卡显示一条简短说明，指向**畴与双晶**选项卡和**新反射**选项卡 — 它们已经为那些情形承载了失去的点阵对称性信息。

**右 — 一般位置**：在 $G$ 之下，图中所有的点构成单一轨道（一个一般位置）；在 $H$ 之下，该轨道分裂为 $[G:H]$ 个子轨道（可与**轨道分裂**选项卡对照）。包含代表点的子轨道 — 即仍作为 $H$ 的一个一般位置留存的那组点 — 画为**黑色**；其余子轨道的点在对称性降低后不再与代表点等价，画为**黄色**。当投影使两个等价点重合时，采用 ITA 标准的 *split circle*（分裂圆）：一个被竖线分成两半的圆，镜像的那一半以逗号标记，每一半旁边附有高度标记（`+`、`½−`、…）。**若一半在 $H$ 中保持而另一半消失，则分割竖线以及消失那一半的逗号与高度标记以黄色绘制**，于是“失去一半”的位置可以直接从图上读出。轨道着色只在同胞关系（*t*- 以及移除带心的 *k*-）下显示；对扩大晶胞的关系，右侧面板改为显示一条简短说明。

对以菱面体轴 (Rho) 设置显示的 R 空间群，两幅图在此投影下都无法绘制；选项卡改为显示一条注记，两幅图均可在同一 R 空间群的六方轴 (Hex) 设置下查看。

![对称要素与一般位置选项卡](../../../assets/cap-zh-Hans-auto/FormGroupRelations-tabElements.png)
