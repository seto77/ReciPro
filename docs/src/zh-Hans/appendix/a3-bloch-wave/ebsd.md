...
master pattern 本身并不是探测器图像；它是晶体固定方向空间中的强度分布。ReciPro 用 Roşca–Lambert 等积投影对该方向空间进行采样，并将 $+Z$ 与 $-Z$ 两个半球存储为各自独立的平面阵列。等积采样减小了两极与赤道之间的密度偏差。
...
根据相反定理，方向 $\widehat{\mathbf s}$ 的 master pattern 被作为沿 $-\widehat{\mathbf s}$ 入射的平面波问题求解，并且对每个方向都把晶体表面视为垂直于 $\widehat{\mathbf s}$。因此其厚度坐标 $t$ 是沿出射方向测量的长度。与此不同，蒙特卡罗给出的源深度 $z$ 是沿倾斜样品表面的法线方向测量的深度。设出射方向与表面法线的夹角为 $\chi$、$\mu=\cos\chi$，则深度为 $z$ 的源沿出射方向到表面的路径长度为

$$t=\frac{z}{\mu}$$

ReciPro 对每个出射方向的分箱和每个能量，用指数分布 $\propto e^{-z/\lambda_z(E)}$ 拟合深度分布，在探测器的每个像素上用该像素的 $\mu$ 换算为 $\lambda_t=\lambda_z/\mu$，再把在深度阵列范围内归一化的分布按 master pattern 的每个厚度区间积分，作为权重。因此上式求和中的深度下标遍历厚度 $t_j$，像素的权重不仅取决于 $\widehat{\mathbf s}$，也取决于该像素的 $\mu$。出射越掠射（$\mu$ 越小），同一深度的源经过的路径越长，权重便移向更厚的切片。

自动设定的深度阵列以到达探测器的电子的路径长度 $z/\mu$ 的第 99.9 百分位（向上取整到 2 位有效数字）为上限，由越靠近表面越密的 40 个不等间距点组成。
