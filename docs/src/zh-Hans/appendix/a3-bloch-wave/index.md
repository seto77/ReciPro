...
本附录概述 ReciPro 的 **衍射模拟器**、**CBED** 与 **HRTEM/STEM** 模拟器所采用的动力学电子衍射理论。ReciPro 遵循 **Bethe / 布洛赫波** 的表述方式。逐步的计算流程（光学势、透射系数、强度）在 [动力学计算（共通核心）](calculation.md) 中说明。
...
将 $\mathbf{A}$ 对角化即可一次性得到 **所有** 布洛赫波矢与振幅。衍射束的振幅 —— 进而其强度 —— 随后由入射面与出射面处的边界条件以及样品厚度确定。这些步骤、光学（复）势、德拜-沃勒因子以及透射系数 $T_{\mathbf g}$ 均在 [动力学计算（共通核心）](calculation.md) 中说明。
