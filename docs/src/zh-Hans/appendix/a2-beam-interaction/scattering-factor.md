...
**如何选择。** 这三者都拟合同一底层的 $f_e(s)$，并在低 $s$ 处高度吻合；它们的差异主要在于适用范围以及原子芯的表示方式。**Peng**（中性原子和常见离子；表格拟合到 $s \le 6$ Å⁻¹，但由于 Gaussian 尾部比真实曲线下降更快，实际上仅在 $s \lesssim 2$ Å⁻¹ 内准确）是 SAED/CBED 结构因子的常用默认选项；**Kirkland** 通过洛伦兹芯项扩展到更高的 $s$，适用于 HRTEM/STEM（注意 $q=2s$）；**8-Gaussians** 适用于达到非常高 $s$ 的反射。对于轻元素，这三者几乎无法区分；差异在重元素的大角度处才显现。
