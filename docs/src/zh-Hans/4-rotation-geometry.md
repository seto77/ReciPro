...
- **Rotation matrix** 显示对应于当前旋转状态的 3×3 矩阵。
...
由当前 (Φ, θ, Ψ) 生成的 3 × 3 矩阵。使用 **Copy to Excel** / **Paste from Excel** 可将矩阵通过电子表格往返传输。
...
| 按钮 | 操作 |
|--------|--------|
| Copy to Excel | 以制表符分隔格式复制 3×3 旋转矩阵 |
| Paste from Excel | 从剪贴板设置旋转矩阵（制表符分隔的 3×3） |
| View along beam | 匹配主窗口的投影（Z 轴垂直于屏幕） |
| Isometric | 切换到等轴测投影 |
...
当勾选 **Link** 时，ReciPro 坐标系与实验坐标系耦合在一起：它们的欧拉角会被调整，使得对象取向在两个系统之间保持一致。
...
4. 勾选 **Link** — 现在，每当您在主窗口中将晶体指向不同取向时，所需的测角仪角度会自动显示。
