...
| 在任意搜索框中按 <kbd>ENTER</kbd> | 执行数据库搜索（与 **Search** 按钮相同） |
| 单击结果表中的某一行 | 将该晶体加载到主窗口 |
| 单击 **Periodic table** 弹出窗口中的某个元素 | 循环切换其筛选状态：*ignore* → *must include* → *must exclude* |
...
显示与搜索条件匹配的晶体。选择某个晶体即可将其传输到主窗口的晶体信息。按 **Add** 或 **Replace** 将其添加到晶体列表。
...
在下面输入搜索条件，然后按 **Search** 按钮或 **Enter** 键。

| 条件 | 说明 |
|-----------|-------------|
| **Name** | 晶体名称 |
| **Element** | 周期表选择器（可包含/必须包含/必须不包含） |
| **Reference** | 标题、期刊、作者 |
| **Crystal system** | 选择晶系 |
| **Cell Param** | 点阵常数及误差 |
| **d-spacing** | 最强反射的 d 值及误差 |
| **Density** | 密度及误差 |
...
按 **Periodic Table** 按钮以打开元素选择器。每个元素按钮在三种状态之间循环切换：
