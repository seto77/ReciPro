...
|--------|------|
| 保存图像 | 将渲染图像保存到文件 |
| 复制主图像到剪贴板 | 复制图像（也可用 Ctrl+Shift+C） |
| 保存视频 | 将旋转/平移动画保存为 MP4（可设置速度、时长、fps、质量等） |
| Export 3D Model (3MF/STL) | 将显示中的结构导出为可用于 3D 打印的实体数据 |
| 快捷键提示 | 显示键盘快捷键 |
...
**Export 3D Model (3MF/STL)** 将屏幕上当前绘制的结构模型导出为可用于 3D 打印的实体数据。只有闭合实体才能导出——原子（球、椭球）、化学键（圆柱）和配位多面体。原子标签、边界面和晶面无法打印，因此被忽略。晶胞边框由线段构成，只有勾选相应选项时才会替换为圆柱（角上加球）后输出。

![Export 3D Model 对话框](../assets/cap-zh-Hans-auto/FormExport3DModel.png)

对话框顶部显示参与导出的对象数与三角形数，以及模型的实际尺寸（**Model size**，nm）。

**Scale** : 输出尺寸的确定方式

- **Fit largest dimension** : 自动确定比例，使模型的最长边达到指定长度（5–500 mm，默认 80 mm）。
- **Fixed scale** : 直接指定比例（mm/nm）。用于按同一比例制作多个晶体并排比较。切换到该项时，会以刚才生效的比例作为初始值。
- 下面一行显示确定后的比例与输出尺寸（**Output size**，mm）。

**Include** : 打印中包含的要素

- **Atoms** / **Bonds** / **Polyhedra** : 只能选择当前显示的种类；取消勾选后仍留在屏幕上，但不会进入打印。
- **Polyhedra: solid faces** / **edges, dia.** : 选择将配位多面体输出为带面的实心体，还是仅棱边的骨架（圆柱＋顶点球，默认 2.0 mm）。骨架可以看到内部的原子。
- **Unit cell edges as cylinders, dia.** : 将晶胞边框替换为指定直径（默认 2.4 mm）的圆柱后包含进来。

**Printability** : 打印适性

- **Thicken bonds thinner than** : 打印后直径低于指定值（默认 1.2 mm）的化学键，会依据原始图元以该直径重新生成，以免细键折断。若关闭该项而存在过细的化学键，则会出现红色警告。

**Format** : 文件格式

- **STL (single color)** : 单色二进制 STL。
- **3MF (parts colored by element)** : 按元素着色的 3MF；颜色名称中带有元素名。

按 **Save...** 会询问文件名。导出结束后，三角形数、输出尺寸与比例会记录在左下角的信息栏中。
