...
**保存影片** 会打开下方的影片设置对话框：设置旋转速度、录制时长和方向（当前投影、方向指数或晶面）、编解码器（H.264 / H.265）及编码速度，然后按 **OK** 生成 MP4 文件。
...
为当前晶体注册的所有边界面。使用 **Add / Replace / Delete** 来操作列表；最左侧的复选框可在不删除某个面的情况下临时禁用它。

> 要永久保存更改，您还必须在 **主窗口** 中按 **Add** 或 **Replace**。否则，下次在主晶体列表中更改选择时，更改将丢失。
...
晶体中的原子列表。使用 **Add / Replace / Delete** 来操作列表；最左侧的复选框可临时隐藏某个原子。

> 要永久保存更改，也请在 **主窗口** 中单击 **Add** 或 **Replace**。

#### 元素和位置

- **Label**：原子的自由文本标签（用于图例和工具提示）。
- **Element**：化学元素 / 电离状态。
...
- **Apply to same elements**：将当前的半径和颜色应用于同一元素种类的每个原子。

### 化学键和多面体

![化学键选项卡](../assets/cap-zh-Hans-auto/FormStructureViewer.splitContainer1.tabControl.tabPageBond.png)

键长阈值、多面体显示、棱边。

#### 化学键列表

为晶体注册的所有化学键/多面体规则。使用 **Add / Replace / Delete**；最左侧的复选框可临时禁用某个条目。与原子和边界一样，要使更改永久生效，需在 **主窗口** 中按 **Add** / **Replace**。

#### 化学键属性

- **Bonding Atom (center)**：用作化学键 / 多面体中心原子的元素种类。
- **Bonding Atom (vertex)**：用作顶点（另一端）的元素种类。
- **Length between … and …**：距离的下限和上限阈值。超出此范围的原子对不会被绘制。
- **Bond Radius**：绘制的化学键粗细（圆柱半径）。
- **Alpha**：化学键透明度（0 = 透明，1 = 不透明）。

#### 多面体属性

- **Show Polyhedron**：勾选时，绘制由当前化学键定义的多面体（仅当中心/顶点组合在几何上有效时）。
- **Inner Bonds**：显示/隐藏多面体内部的化学键。
- **Center Atom**：显示/隐藏中心原子。
- **Vertex Atoms**：显示/隐藏顶点原子。
- **Color** / **Alpha**：面颜色和透明度。
- **Show Edge**：绘制连接各顶点的棱边。
- **Edge Color** / **Width**：棱边的颜色和线宽。
...
![坐标选项卡](../assets/cap-zh-Hans-auto/FormStructureViewer.splitContainer1.tabControl.tabPageCoordinateInformation.png)
...
![信息选项卡](../assets/cap-zh-Hans-auto/FormStructureViewer.splitContainer1.tabControl.tabPageInformation.png)
...
将投影中心设置为指定的坐标。打开 **Custom** 以输入任意坐标。
...
| Labels | 切换原子标签 |
| Unit Cell | 切换晶胞棱边 |
| Sym. Elems. | 切换对称元素叠加层（见上文） |
| Reset Rotation | 返回初始取向 |
| Like Vesta | Vesta 风格的外观 |
