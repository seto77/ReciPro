...
**保存 ▸ Candidate list (CSV)...** 可导出指数标定得到的取向候选，**复制 ▸ Candidate list (TSV)** 则把同一表格放入剪贴板：每个候选 × 晶粒一行，包含排名、晶粒序号、晶体名称、ZXZ 欧拉角（°）、旋转矩阵的九个分量、均方残差（nm⁻²）、已分配衍射斑数以及衍射斑 → (hkl) 的对应关系。CSV 的小数点始终为句点；TSV 可直接粘贴到电子表格中。
