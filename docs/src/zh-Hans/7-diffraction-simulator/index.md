...
| 方向键 — **TEM 样品台** | 逐步改变样品台倾角（先勾选 **启用方向键**；无论 ReciPro 的哪个窗口在前台均有效） |
...
打开一个窗口，将衍射图样与双倾（或旋转）**TEM 样品台**联动。设置样品台倾角会更新图样和晶体取向，可达取向可在极射赤平投影上显示（v4.914 新增）。在极射赤平投影上左键双击会将样品台倾角设为该点，勾选 **启用方向键** 后可用方向键逐步改变倾角。只要此窗口处于打开状态，无论 ReciPro 的哪个窗口位于前台，方向键均有效。
...
![菊池线选项卡（动力学模式）](../../assets/cap-zh-Hans-auto/FormDiffractionSimulator.toolStripContainer1.panelMain.tabControl.tabPageKikuchi-dynamical.png)

在工具栏启用菊池线时生效。

- **反射选择** : 选择由哪些反射生成菊池线。可选 **结构因子**（按 $\lvert F_{hkl}\rvert$ 排名靠前的 *N* 个反射）或 **1/d 截止**（所有 1/d 低于阈值（nm⁻¹）的反射）。
- **模式** : 菊池线的绘制方式。组合框下方的设置随模式切换。
    - **几何** : 所有选中的线以同一固定颜色绘制。
    - **运动学** : 线的浓淡按各反射的运动学相对强度变化。
    - **动力学** : 以热漫散射为源的双束动力学过剩/亏损带轮廓，在后台线程中计算。过剩带出现在高角侧，亏损特征朝向直射束一侧；试样厚度与强度设置共用。计算期间先以几何线代替显示。
- **几何 / 运动学的设置** : **菊池线颜色** 和 **线宽**。
- **动力学的设置** :
    - **过剩侧** / **亏损侧** : 带的明亮侧（对比度为正）与暗侧（对比度为负）的颜色。
    - **带数** : 以动力学轮廓绘制的带数（按动力学对比度取前 *N* 条；1〜50，默认 30）。
    - **质量** : 横穿每条带的采样点数（快速 65 / 标准 129 / 高 257）。
    - **对比度** : 压缩前施加到带对比度上的显示增益。
    - **标度** : 亮度压缩曲线（Linear / Log / Tanh）。均为可视化变换，并非物理相机响应。
    - **锁定亮度** : 冻结亮度归一化，例如用于比较不同取向或厚度。
    - 下方灰色注记显示源与吸收不一致的提示、绘制的带数及诊断信息（例如对 *B* = 0 的原子代用默认 *B* 因子）。
