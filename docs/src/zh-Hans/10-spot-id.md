...
| Refine thickness and direction | 应用动力学理论（布洛赫波法，Bethe method）来精修最符合检测到强度的样品厚度和晶体取向 |
...
**Three sides mode** : 输入以 direct spot 为一个顶点的三角形三条边的长度。

**Two sides and an angle mode** : 输入两条边的长度（含 direct spot）以及它们之间的夹角。
