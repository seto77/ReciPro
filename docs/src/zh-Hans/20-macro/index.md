...
上面的截图开启了 **Show samples**，因此显示出内置的示例宏。宏列表位于左侧，代码编辑器位于右侧，底部是内置函数帮助表。
...
开启 **Show samples**（左上角）会用内置的示例宏临时替换你的宏列表（基本循环与条件判断、数学函数、旋转/取向对齐、遍历晶体列表、衍射/图像模拟、倾斜/能量系列、衍射点信息等）。这些示例为只读，并以当前 UI 语言显示；可用于学习，或作为复制的起点。关闭后会恢复你自己的宏。
