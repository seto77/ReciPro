...
| 화살표 키 — **TEM 홀더** | 홀더 기울기를 단계적으로 변경(먼저 **화살표 키 사용** 체크; ReciPro의 어느 창이 앞에 있어도 동작) |
...
회절 패턴을 더블 틸트(또는 회전) **TEM 홀더**와 연동하는 창을 엽니다. 홀더 기울기 각도를 설정하면 패턴과 결정 방위가 갱신되고, 도달 가능한 방위를 스테레오넷에 표시할 수 있습니다(v4.914에서 추가됨). 스테레오넷을 왼쪽 더블 클릭하면 홀더 기울기가 그 지점으로 설정되며, **화살표 키 사용**를 체크하면 화살표 키로 기울기를 단계적으로 변경할 수 있습니다. 이 창이 열려 있는 동안에는 ReciPro의 어느 창이 앞에 있어도 화살표 키가 동작합니다.
...
![키쿠치 탭 (동역학적 모드)](../../assets/cap-ko-auto/FormDiffractionSimulator.toolStripContainer1.panelMain.tabControl.tabPageKikuchi-dynamical.png)
...
- **모드 (Mode)** : 키쿠치 선을 그리는 방법입니다. 콤보 박스 아래의 설정은 모드에 따라 바뀝니다.
    - **기하학적 (Geometric)** : 선택된 모든 선을 하나의 고정 색으로 그립니다.
    - **운동학적 (Kinematical)** : 선의 진하기가 각 반사의 운동학적 상대 강도를 따릅니다.
    - **동역학적 (Dynamical)** : 열확산산란을 소스로 하는 2빔 동역학적 과잉/결손 밴드 프로파일을 백그라운드 스레드에서 계산하여 그립니다. 과잉 밴드는 고각 쪽에, 결손 특징은 직접빔 쪽에 나타납니다. 시료 두께는 강도 설정과 공유됩니다. 계산 중에는 기하학적 선이 대신 그려집니다.
- **기하학적 / 운동학적 설정** : **키쿠치 선 색** 과 **선 너비**.
- **동역학적 설정** :
    - **과잉** / **결손** : 밴드의 밝은 쪽(대비 양수)과 어두운 쪽(대비 음수)의 색.
    - **밴드 수 (Bands)** : 동역학적 프로파일로 그리는 밴드 수 (동역학적 대비 상위 *N*개; 1〜50, 기본 30).
    - **품질 (Quality)** : 각 밴드를 가로지르는 샘플 점 수 (고속 65 / 표준 129 / 고품질 257).
    - **대비 (Contrast)** : 압축 전에 밴드 대비에 곱하는 표시용 이득.
    - **스케일 (Scale)** : 밝기 압축 곡선 (Linear / Log / Tanh). 모두 표시용 변환이며 물리적 카메라 응답이 아닙니다.
    - **밝기 고정 (Lock brightness)** : 밝기 정규화를 고정합니다 (예: 방위나 두께를 바꿔 밝기를 비교할 때).
    - 아래 회색 주석에는 소스/손실 불일치 태그, 그려진 밴드 수, 진단 정보(예: *B* = 0 원자에 기본 *B* 인자 대용)가 표시됩니다.
...
- **Effective digit** : 표에 표시되는 유효 숫자 자릿수. **Auto resize row width** : 열 너비를 자동으로 맞춤. **Copy to clipboard** : 표를 스프레드시트에 붙여 넣을 수 있는 텍스트로 내보냅니다. **Save as CSV** : 같은 표(표시 중인 열)를 CSV 파일로 저장합니다. 소수점은 항상 마침표이므로 소수점이 쉼표인 로캘에서도 파일이 깨지지 않습니다. (이 폼은 일본어 UI에서도 영어로 표시됩니다.)
