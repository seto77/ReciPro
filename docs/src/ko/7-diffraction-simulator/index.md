...
| 화살표 키 — **TEM 홀더** | 홀더 기울기를 단계적으로 변경(먼저 **화살표 키 사용** 체크) |
...
회절 패턴을 더블 틸트(또는 회전) **TEM 홀더**와 연동하는 창을 엽니다. 홀더 기울기 각도를 설정하면 패턴과 결정 방위가 갱신되고, 도달 가능한 방위를 스테레오넷에 표시할 수 있습니다(v4.914에서 추가됨). 스테레오넷을 왼쪽 더블 클릭하면 홀더 기울기가 그 지점으로 설정되며, **화살표 키 사용**를 체크하면 화살표 키로 기울기를 단계적으로 변경할 수 있습니다.
