...
| 메뉴 항목 | 설명 |
|-----------|------|
| 이미지 저장 | 렌더링된 이미지를 파일로 저장 |
| 주 이미지를 클립보드에 복사 | 이미지를 복사(Ctrl+Shift+C 로도 가능) |
| 동영상 저장 | 회전·평행 이동 동영상을 MP4 로 저장(속도·길이·fps·품질 등을 설정) |
| Export 3D Model (3MF/STL) | 표시 중인 구조를 3D 프린팅용 입체 데이터로 내보내기 |
| 단축키 힌트 | 키보드 단축키를 표시 |
...
**Export 3D Model (3MF/STL)** 은 화면에 그려져 있는 구조 모델을 3D 프린팅용 입체 데이터로 내보냅니다. 내보낼 수 있는 것은 닫힌 입체뿐으로, 원자(구·타원체)·결합(원기둥)·배위 다면체가 대상입니다. 원자 라벨·경계면·격자면은 인쇄할 수 없으므로 무시됩니다. 단위 격자 테두리는 선분이므로, 해당 옵션을 켰을 때만 원기둥(모서리에는 구)으로 바꾸어 출력합니다.

![Export 3D Model 대화 상자](../assets/cap-ko-auto/FormExport3DModel.png)

대화 상자 위쪽에는 대상이 되는 오브젝트 수·삼각형 수와 모델의 실제 크기(**Model size**, nm)가 표시됩니다.

**Scale** : 출력 크기를 정하는 방법

- **Fit largest dimension** : 모델의 가장 긴 변이 지정한 길이(5–500 mm, 기본 80 mm)가 되도록 배율을 자동으로 정합니다.
- **Fixed scale** : 배율(mm/nm)을 직접 지정합니다. 여러 결정을 같은 축척으로 만들어 나란히 놓고 비교할 때 사용합니다. 이쪽으로 전환하면 직전의 실효 배율이 초깃값으로 들어갑니다.
- 아래 줄에 결정된 배율과 출력 치수(**Output size**, mm)가 표시됩니다.

**Include** : 인쇄에 포함할 요소

- **Atoms** / **Bonds** / **Polyhedra** : 화면에 표시되어 있는 종류만 선택할 수 있으며, 체크를 해제하면 화면에는 남겨 둔 채 인쇄에서만 제외됩니다.
- **Polyhedra: solid faces** / **edges, dia.** : 배위 다면체를 면이 있는 속이 찬 입체로 낼지, 모서리만의 골격(원기둥＋꼭짓점의 구, 기본 2.0 mm)으로 낼지 선택합니다. 골격으로 하면 안쪽 원자가 보입니다.
- **Unit cell edges as cylinders, dia.** : 단위 격자 테두리를 지정한 지름(기본 2.4 mm)의 원기둥으로 바꾸어 포함합니다.

**Printability** : 인쇄 적합성

- **Thicken bonds thinner than** : 출력 후 지름이 지정값(기본 1.2 mm)에 못 미치는 결합을 원래 프리미티브로부터 그 지름으로 다시 만듭니다. 가는 결합이 부러지는 것을 막기 위한 옵션입니다. 끈 채로 너무 가는 결합이 있으면 빨간색 경고가 표시됩니다.

**Format** : 파일 형식

- **STL (single color)** : 단색 바이너리 STL.
- **3MF (parts colored by element)** : 원소별로 색을 나눈 3MF. 색 이름에는 원소 이름이 들어갑니다.

**Save...** 를 누르면 파일 이름을 묻습니다. 내보내기가 끝나면 삼각형 수·출력 치수·배율이 왼쪽 아래 정보란에 기록됩니다.
