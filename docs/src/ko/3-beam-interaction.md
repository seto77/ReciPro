...
이 창에는 특별한 키 조합이 없습니다. <kbd>F1</kbd> 은 이 매뉴얼 페이지를 엽니다. **산란 인자** 탭에서는 세로 커서선을 **끌어서** 그 위치에서 각 원소의 산란 인자를 읽을 수 있으며, 모든 탭에는 테이블을 스프레드시트에 붙여넣을 수 있는 텍스트로 내보내는 **Copy** 버튼이 있습니다.
...
- **X-ray** 의 경우 **Element** (양극 재료) 와 특성선 (Kα 등) 을 선택하면 해당 특성 X선의 파장이 자동으로 설정됩니다. **f(s)**를 선택하면 **곡선을 어디에서 가져올지** 고르는 라디오 버튼 행이 하나 더 나타납니다 — **Waasmaier–Kirfel**(기본 적합. Z = 1–98의 이온도 포함) 또는 **Temari**. **F(q)+S(q)**를 표시하는 동안에는 그 두 곡선이 xraylib 고유이며 출처를 고를 수 없으므로 흐리게 표시됩니다.
...
| 빔 | 반사 | 감쇠 & 수송 | 산란 인자 | 형광 |
|------|------|------|------|------|
| **X-ray** | 이상 분산을 포함한 구조 인자 | µ/ρ, µ, 투과율 + 흡수단 (에너지에 대해) | $f(s)$(Waasmaier–Kirfel 또는 Temari) 또는 $F(q)+S(q)$ | 특성선 + EDX 스틱 |
| **Electron** | 전자 구조 인자 | σ, MFP, \|dE/ds\|, IMFP, 비정 (에너지에 대해) | Peng / Kirkland / 8-Gaussians / Temari | — (숨김) |
...
## 산란 인자 탭 {#scattering-factors-tab}

각 구성 원소의 원자 산란 인자를 $s = \sin\theta/\lambda$ (Å⁻¹) 에 대해 플롯합니다. 각 원소는 자체 색으로 그려지며, **세로 커서선** 을 끌어서 그 위치에서 모든 원소의 산란 인자를 왼쪽 테이블로 읽을 수 있습니다.

=== "X-ray"
    ![산란 인자 — X-ray](../assets/cap-ko-auto/FormBeamInteraction-xray-scattering.png)

=== "Electron"
    ![산란 인자 — electron](../assets/cap-ko-auto/FormBeamInteraction-electron-scattering.png)

=== "Neutron"
    ![산란 인자 — neutron](../assets/cap-ko-auto/FormBeamInteraction-neutron-scattering.png)

- **X-ray** 는 두 가지 **Model** 모드를 제공합니다: **f(s)** 는 통상적인 X선 원자 산란 인자 (전자 단위) 를 플롯합니다. **F(q)+S(q)** 는 Rayleigh **가간섭** 형상 인자 $F(q)$ 와 함께 Compton **비가간섭** 산란 함수 $S(q)$ 를 (xraylib에서) 플롯합니다. 테이블은 또한 현재 에너지에서의 이상 분산 항 **f'(E)** 와 **f''(E)**, 그리고 f(s) 열이 어느 출처에서 왔는지 나타내는 **모델** 열을 나열합니다.
- **Electron** 은 전자 산란 인자의 네 가지 출처를 제공합니다: **Peng**, **Kirkland**, **8-Gaussians**, **Temari**. 테이블은 $f_e(s)$ (nm) 와 그것을 생성한 **model** 을 보여줍니다.
- **Neutron** 산란 길이는 $s$ 에 의존하지 않으므로 곡선이 그려지지 않습니다. 테이블은 각 원소의 가간섭 산란 길이 **b_coh** 와 그 가간섭 / 비가간섭 단면적을 나열합니다.
- **Temari**는 적합이 아니라 계산된 표입니다(Temari dataset-factors v1.0.0, CC-BY-4.0. 완전 Dirac 자기무모순장 계산에서 얻음). **중성 원자 Z = 1–86만** 지원하며, 이온과 Z = 87–98에는 값이 없습니다. 중성 원자 값으로 채우지 않고 곡선을 그리지 않으며 표의 셀은 비워 둡니다. Temari를 선택한 동안에는 그 사실을 알리는 한 줄 안내가 표시됩니다.
- **Debye-Waller** 는 각 원자의 등방성 변위 매개변수를 사용하여 각 인자에 열적 감쇠 $e^{-B s^2}$ 를 곱합니다.

---

## 형광 탭 {#fluorescence-tab}

X선 빔에 대해 시료의 특성 **형광** 방출을 나타냅니다. (이 탭은 전자 및 중성자 빔에서는 숨겨집니다.)

![형광 (X-ray)](../assets/cap-ko-auto/FormBeamInteraction-xray-fluorescence.png)
