...
위 스크린샷에서는 **Show samples**가 켜져 있어 내장 샘플 매크로가 표시됩니다. 매크로 목록은 왼쪽에, 코드 편집기는 오른쪽에, 내장 함수 도움말 표는 아래쪽에 있습니다.
...
- **접근**: 메인 창에서 Macro 버튼을 클릭하여 매크로 편집기를 엽니다
...
**Show samples**(왼쪽 위)를 켜면 매크로 목록이 일시적으로 내장 샘플 매크로(기본 루프와 조건문, 수학 함수, 회전/정렬, 결정 목록 순회, 회절/이미지 시뮬레이션, 기울기/에너지 시리즈, 반사 정보 등)로 대체됩니다. 샘플은 읽기 전용이며 현재 UI 언어로 표시됩니다. 학습용으로 또는 복사하여 출발점으로 사용하세요. 끄면 자신의 매크로가 복원됩니다.
