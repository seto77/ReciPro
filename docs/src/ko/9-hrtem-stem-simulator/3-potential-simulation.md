...
> **퍼텐셜 모드에서는 시료 두께, 강도 정규화, 이미지 모드(단일/연속) 패널이 표시되지 않습니다.** TEM 조건 중에서는 가속 전압만 활성화됩니다.
...
| 종류 | 설명 |
|------|-------------|
| **$U_g$ — 탄성 산란 퍼텐셜** | 탄성 산란을 일으키는 (정전기적) 결정 퍼텐셜입니다. 산란 강도를 나타냅니다 |
| **$U'_g$ — 흡수 퍼텐셜** | 열 확산 산란(TDS)에서 발생하는 허수(흡수) 퍼텐셜입니다. 탄성 채널로부터의 손실을 나타냅니다 |

$U_g$와 $U'_g$는 동시에 표시할 수 있습니다(체크된 각 항목마다 패널이 하나씩 추가됩니다).
...
밝기(Min / Max), 색상 척도 및 단위 격자 그리드 오버레이는 왼쪽의 **Adjust**와 **Display**에서 설정합니다(자세한 내용은 [개요 페이지](index.md#display-settings) 참조).
