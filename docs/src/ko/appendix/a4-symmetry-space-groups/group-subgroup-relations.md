...
브라우저의 다른 부분이 *공간*군 사이를 오가는 것과 달리, 이 탭은 그 이동이 놓인 더 넓은 지도를 보여 줍니다: **32개 결정학적 점군 유형(기하학적 결정류)의 Hasse 도표**, 즉 어느 유형이 어느 유형의 부분군으로 나타나는지에 대한 반순서(partial order)입니다. 각 노드는 하나의 점군 유형이고, 세로축은 군의 위수(맨 아래 1, 맨 위 48, 로그 눈금)이며, 육방·삼방정족이 왼쪽 탑을, 입방–정방–사방정 탑이 오른쪽을 이룹니다. 변(edge)은 *극대*(피복) 부분군 관계 — 그 사이에 제3의 유형이 엄격하게 끼어들지 않는 관계 — 이며, 32개 유형 사이에 정확히 80개가 있습니다. 이 도표는 반순서이지만 수학적 의미의 격자(lattice)는 *아닙니다*: 서로 비교할 수 없는 두 개의 극대 원소 $m\bar3m$과 $6/mmm$을 갖습니다.

- 열람 중인 공간군의 점군에는 (계통도 탭의 현재 노드와 마찬가지로) 옅은 파란색 후광이 붙으며, 기본값으로 이것이 *포커스* 유형입니다.
- 임의의 노드를 **클릭**하면 대신 그 유형을 포커스합니다: 그 **부분군 유형은 파란색으로**, **초군 유형은 주황색으로** 강조되고, 포커스된 노드 자신의 변에 표시되는 숫자는 그 극대 단계의 **지수**(위수 비)입니다. 포커스와 무관한 유형은 회색으로 남습니다. 빈 공간을 클릭하면 포커스가 현재 점군으로 돌아갑니다. (클릭해도 어디로도 이동하지 않습니다 — 하나의 점군 유형은 하나가 아니라 다수의 공간군에 대응하기 때문입니다.)
- 상단의 캡션은 포커스된 유형을 요약합니다: Hermann–Mauguin 및 Schoenflies 기호, 군의 위수 $|G|$, 230개 공간군 유형 중 몇 개가 이 점군에 속하는지, 그리고 그 부분군/초군 집합의 크기.

이 도표는 Hermann의 정리가 점군에 드리운 그림자입니다: 트리에서의 *t*-부분군 한 단계는 여기서 변을 정확히 하나 내려가는 것에 대응하며(*t*-단계의 공간군 지수는 그 변의 점군 지수와 같습니다), *k*- 및 동형 단계는 같은 노드에 머뭅니다.

![점군 탭](../../../assets/cap-ko-auto/FormGroupRelations-tabPointGroups.png)

---

## 대칭 요소·일반 위치 탭

**변환 행렬** 탭이 유지·소실 연산을 표로 나열하는 반면, 이 탭은 같은 정보를 *기하학적으로* — 결정학자가 이미 ITA Vol. A에서 나란히 놓고 읽어 온 두 장의 그림으로 — 보여 줍니다: 왼쪽은 부모의 [대칭 요소 다이어그램](symbols-and-diagrams.md#symmetry-element-diagram)(축·면·반전 중심), 오른쪽은 부모의 **일반 위치 다이어그램**입니다. 두 그림 모두 하나의 배색 규칙 — **$H$에서 살아남는 것은 검은색, 소실되는 것은 노란색**(단위 격자는 그 밑에 파란색으로 그려집니다) — 을 따르므로, 결정이 $G$에서 부분군 $H$로 변태할 때 어떤 대칭 요소가 깨지고 일반 위치의 궤도가 어떻게 분열하는지를 이 탭에서 한눈에 알 수 있습니다. 투영 방향(⟂ *a*, *b* 또는 *c*)은 부모의 결정계에 맞춰 자동으로 선택되며, 헤더에는 관계, 그 종류와 지수, 그리고 투영 방향이 표시됩니다.

**왼쪽 — 대칭 요소.**

- 겹쳐 그리기는 소실되는 부모의 요소를 노란색으로 그린 위에, $H$ 자신의 연산 집합으로부터 직접 재구성한 대칭 요소를 검은색으로 그립니다 — 따라서 유지되는 요소란 기호를 하나하나 추측하는 일 없이 정확히 $H$에서 다시 나타나는 요소입니다.
- 대칭 요소가 더 낮은 요소로 격하되는 경우에는 살아남는 쪽 기호만 그려집니다: 예컨대 4회 축이 2회 축만 남기는 위치에는 **검은 2회 기호**만 나타납니다. 소실된 요소가 노란색으로 표시되는 것은 그 기하학적 위치에 아무것도 살아남지 않는 경우뿐이어서, 남은 대칭을 읽기 쉬운 상태로 유지합니다.

왼쪽 그림은 부분군의 단위 격자가 부모의 단위 격자와 어떻게 관련되는지에 따라 세 가지 경우를 다룹니다.

- **동일한 관용 단위 격자** — *translationengleiche*(*t*-) 부분군, 그리고 중심화 벡터만 제거하는 *klassengleiche*(*k*-) 부분군(부분 격자 기저의 행렬식 $\det = 1$)입니다. 이때 $H$의 요소가 부모 격자 안에 놓이므로, 겹쳐 그리기는 부모 격자 하나의 그림이 됩니다. (중심화를 제거하는 *k*- 관계, 예를 들어 $I$ 심 또는 $F$ 심 $\to$ 단순 격자에서는, 중심화가 만들어 낸 나선축과 글라이드 면이 노랗게 됩니다.)
- **면내 격자 확대** — 확대가 *a*–*b* 면내에 있는($a'=n_a a$, $b'=n_b b$, *c*는 불변) 사방정·정방정·입방정 부모의 *k*- 또는 동형 관계입니다. 확대된 격자를 (⟂ *c*로 투영하여) $n_a\times n_b$개의 부모 격자 타일 격자로 그리고, **각 타일을 개별적으로 채색**합니다. $H$에서 유지되는 대칭 요소는 그 복사본이 살아남는 타일에서는 검은색, 격자 확대로 제거된 곳에서는 노란색으로 나타납니다 — 그리하여 격자 배가가 하나 걸러 하나씩 복사본이 노랗게 되는 형태로 곧바로 드러납니다.
- **그 밖의 경우**(시선 축 $c$ 방향으로의 확대, 사교/비직교 격자 변환, 육방정·삼방정·단사정 부모)에는 잃어버린 대칭을 2차원 투영에서 모호함 없이 그릴 수 없으므로, 탭에는 **도메인·쌍정** 탭과 **새 반사** 탭을 가리키는 짧은 안내가 표시되며, 이들 탭은 그러한 경우에 대해 잃어버린 격자 대칭 정보를 이미 담고 있습니다.

**오른쪽 — 일반 위치.** $G$ 아래에서는 그려진 모든 점이 하나의 궤도(하나의 일반 위치)를 이루지만, $H$ 아래에서는 그 궤도가 $[G:H]$개의 부분 궤도로 분열합니다(**궤도 분열** 탭과 비교). 대표점을 포함하는 부분 궤도 — $H$의 하나의 일반 위치로 남는 집합 — 는 **검은색**으로 그려지고, 대칭이 낮아지면서 더는 그것과 등가가 아니게 된 나머지 부분 궤도의 점들은 **노란색**입니다. 투영에 의해 등가인 두 점이 겹치는 위치에는 ITA 표준의 *split circle*을 씁니다: 세로선으로 이등분한 원으로, 거울상 쪽 절반에는 콤마가 찍히고 각 절반 옆에는 높이 라벨(`+`, `½−`, …)이 붙습니다. **한쪽 절반만 $H$에서 살아남고 다른 쪽이 소실되면, 세로 분할선과 소실된 절반의 콤마·높이 라벨은 노란색**으로 그려지므로, 반만 소실된 위치도 그림에서 직접 읽어 낼 수 있습니다. 궤도의 색 구분은 동일 격자 관계(*t*-, 그리고 중심화를 제거하는 *k*-)에서 표시되며, 격자 확대를 동반하는 관계에서는 오른쪽 패널에 짧은 안내가 대신 표시됩니다.

능면체 축 (Rho) 설정으로 표시 중인 R 공간군에서는 두 그림 모두 이 투영으로는 그릴 수 없어 탭에 안내문이 대신 표시되며, 두 다이어그램은 동일한 R 공간군의 육방 축 (Hex) 설정에서 볼 수 있습니다.

![대칭 요소·일반 위치 탭](../../../assets/cap-ko-auto/FormGroupRelations-tabElements.png)
