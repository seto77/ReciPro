...
이 탭은 ***translationengleiche*(*t*-) 부분군에 대해서만** 제공되며, 이때 $H$는 부모의 병진 격자를 유지하므로($T_H=T_G$) 두 군의 요소가 같은 단위 격자 안에 놓입니다. *k*- 및 동형 관계에서는 단위 격자가 커지므로 이 그림은 이후 버전으로 미뤄져 있습니다; 탭에는 **도메인·쌍정** 탭과 **새 반사** 탭을 가리키는 짧은 안내가 표시되며, 이들 탭은 그러한 경우에 대해 잃어버린 격자 대칭 정보를 이미 담고 있습니다.
