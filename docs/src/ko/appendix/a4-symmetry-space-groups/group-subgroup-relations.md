...
**변환 행렬** 탭이 유지·소실 연산을 표로 나열하는 반면, 이 탭은 같은 정보를 *기하학적으로*, 부모의 [대칭 요소 다이어그램](symbols-and-diagrams.md#symmetry-element-diagram)(축·면·반전 중심을 갖춘 ITA Vol. A 양식의 그림) 위에 겹쳐 보여 줍니다. 결정이 $G$에서 부분군 $H$로 변태할 때 "어떤 대칭 요소가 살아남고 어떤 것이 깨지는가"라는 물음에 — 결정학자가 이미 익숙하게 읽어 온 바로 그 그림 안에서 — 한눈에 답합니다.

- **$H$에서 유지되는 요소는 검은색으로, 소실되는 요소는 빨간색으로 그려집니다.** 투영 방향(⟂ *a*, *b* 또는 *c*)은 부모의 결정계에 맞춰 자동으로 선택되며, 헤더에는 관계와 투영이 명시됩니다.
- 어떤 대칭 요소가 더 낮은 요소로 격하되는 경우는 문자 그대로 표현됩니다: 예컨대 2회 축으로 낮아지는 4회 축은 **그 위에 검은 2회 기호를 겹쳐 그린 빨간 4회 기호**로 나타납니다 — 빨간색은 "4회 축은 사라졌다", 검은색은 "여기에 2회 축이 남는다"를 뜻합니다.
- 이 겹쳐 그리기는 먼저 부모 다이어그램 전체를 (*소실* 기준선으로) 그린 다음, 그 위에 $H$ 자신의 연산 집합으로부터 직접 재구성한 대칭 요소를 다시 그려 만듭니다 — 따라서 유지되는 요소란 기호를 하나하나 추측하는 일 없이 정확히 $H$에서 다시 나타나는 요소입니다.

이 탭은 부분군의 단위 격자가 부모의 단위 격자와 어떻게 관련되는지에 따라 세 가지 경우를 다룹니다.

- **동일한 관용 격자** — *translationengleiche*(*t*-) 부분군, 그리고 심맞춤 벡터만 제거하는 *klassengleiche*(*k*-) 부분군(부분 격자 기저의 행렬식 $= 1$)입니다. 이때 $H$의 요소가 부모 격자 안에 놓이므로, 겹쳐 그리기는 단일 격자 그림이 됩니다. (심맞춤을 제거하는 *k*- 관계, 예를 들어 $I$ 심 또는 $F$ 심 $\to$ 단순 격자에서는, 심맞춤이 만들어 낸 나선축과 미끄럼면이 빨갛게 됩니다.)
- **면내 격자 확대** — 확대가 *a*–*b* 면내에 있는($a'=n_a a$, $b'=n_b b$, *c*는 불변) 직방정·정방정·입방정 부모의 *k*- 또는 동형 관계입니다. 확대된 격자를 (⟂ *c* 로 투영하여) $n_a\times n_b$ 개의 부모 격자 타일 격자로 그리고, **각 타일을 개별적으로 채색**합니다. $H$에서 보존되는 대칭 요소는 그 복사본이 살아남는 타일에서는 검정, 격자 확대로 제거된 곳에서는 빨강으로 나타납니다 — 그리하여 격자 배가가 하나 걸러 하나씩 복사본이 빨갛게 되는 형태로 곧바로 드러납니다.
...
![대칭 요소 탭](../../../assets/cap-ko-auto/FormGroupRelations-tabElements.png)
