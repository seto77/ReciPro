...
*t*-부분군 전이에 대해, $G$에서는 계통적으로 소멸했지만 $H$에서는 대칭성상 허용되는 반사($|h|,|k|,|l|\le4$까지 탐색)를 나열합니다 — 즉 부모의 반사 조건([조건](../../2-symmetry-information.md) 탭 참조)은 금지하지만 $H$의 조건은 금지하지 않는 반사입니다.
...
***k*- 또는 동형 관계**에서는 이 탭이 새 반사를 **확대된 부분군 셀로 지수화하여**(역시 $|h'|,|k'|,|l'|\le4$까지) 나열하고, 마지막 열에서 각각을 분류합니다:
...
- **동형 부분군은 index ≤ 4까지만 열거됩니다.** 동형 계열은 더 높은 지수로 한없이 이어지므로(입방정군에서는 $a'=3a$만으로 이미 지수 27), 트리는 목록이 완전한 척하는 대신 해당 가지에 회색의 *"index ≤ 4만 표시 — 동형 계열은 더 높은 지수로 이어집니다"* 알림을 붙입니다.
...
- 반사 탐색 범위는 $|h|,|k|,|l|\le4$로 고정되어 있으며, 현재 버전에서는 사용자가 조정할 수 없습니다.
