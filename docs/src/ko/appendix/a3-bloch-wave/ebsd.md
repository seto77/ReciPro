...
master pattern은 검출기 영상 자체가 아니라, 결정 고정 방향 공간에서의 강도 분포이다. ReciPro는 면적 보존 Roşca–Lambert 투영으로 그 방향 공간을 샘플링하고, $+Z$ 와 $-Z$ 반구를 별도의 평면 배열로 저장한다. 면적 보존 샘플링은 극과 적도 사이의 밀도 편향을 줄인다.
...
상반 정리에 의해 방향 $\widehat{\mathbf s}$ 의 master pattern은 $-\widehat{\mathbf s}$ 방향으로 입사하는 평면파 문제로서, 방향마다 결정 표면을 $\widehat{\mathbf s}$ 에 수직으로 보고 풀린다. 따라서 두께 좌표 $t$ 는 출사 방향을 따라 잰 길이이다. 반면 몬테카를로가 주는 선원 깊이 $z$ 는 기울어진 시료 표면의 법선 방향으로 잰 깊이이다. 출사 방향과 표면 법선이 이루는 각을 $\chi$, $\mu=\cos\chi$ 라 하면, 깊이 $z$ 의 선원에서 표면까지 출사 방향을 따른 경로 길이는

$$t=\frac{z}{\mu}$$

이다. ReciPro는 출사 방향 빈과 에너지마다 깊이 분포를 지수 분포 $\propto e^{-z/\lambda_z(E)}$ 로 맞추고, 검출기의 각 픽셀에서 그 픽셀의 $\mu$ 로 $\lambda_t=\lambda_z/\mu$ 로 환산한 뒤, 깊이 배열 범위에서 정규화한 분포를 master pattern의 두께 구간마다 적분하여 가중치로 삼는다. 따라서 위 합의 깊이 첨자는 두께 $t_j$ 를 따라 움직이고, 픽셀의 가중치는 $\widehat{\mathbf s}$ 뿐 아니라 그 픽셀의 $\mu$ 에도 의존한다. 표면을 스치듯 출사할수록($\mu$ 가 작을수록) 같은 깊이의 선원도 더 긴 경로를 지나므로, 가중치는 두꺼운 슬라이스 쪽으로 옮겨간다.

자동 설정되는 깊이 배열은 검출기에 도달하는 전자의 경로 길이 $z/\mu$ 의 99.9 백분위(유효숫자 2자리로 올림)를 상한으로 하며, 표면에 가까울수록 촘촘한 40점의 부등간격이다.
