...
이 부록은 ReciPro의 **회절 시뮬레이터**, **CBED**, **HRTEM/STEM** 시뮬레이터가 사용하는 동역학적 전자 회절 이론의 개요를 제공합니다. ReciPro는 **Bethe / 블로흐파** 정식화를 따릅니다. 단계별 계산(광학 퍼텐셜, 투과 계수, 강도)은 [동역학적 계산 (공통 코어)](calculation.md)에 설명되어 있습니다.
...
$\mathbf{A}$를 대각화하면 **모든** 블로흐 파동벡터와 진폭을 한 번에 얻습니다. 회절빔의 진폭 — 따라서 강도 — 은 입사면과 출사면에서의 경계 조건 및 시료 두께로부터 따라옵니다. 그 단계들, 광학(복소) 퍼텐셜, 디바이-월러 인자, 투과 계수 $T_{\mathbf g}$는 [동역학적 계산 (공통 코어)](calculation.md)에 설명되어 있습니다.
