...
| Arrow keys — **TEM holder** | Step the holder tilt (tick **Enable arrow keys** first; works with any ReciPro window in front) |
...
- **Effective digit** : number of significant digits shown in the table. **Auto resize row width** : auto-fit column widths. **Copy to clipboard** : exports the table as text that can be pasted into a spreadsheet. **Save as CSV** : writes the same table (the visible columns) to a CSV file; the decimal separator is always a period, so the file stays valid in comma-decimal locales. (This form is shown in English even under a Japanese UI.)
