...
ReciPro offers four sources for $f_e(s)$ — three fitted parametrisations and one computed table:
...
- **Temari** : not a fit at all, but a table computed from first principles (a full Dirac self-consistent-field calculation with the KLI approximation to exact exchange; Temari dataset-factors v2.0.0, CC-BY-4.0). Because a sum of Gaussians decays as $e^{-b s^2}$ while the true $f_e$ falls as $s^{-2}$, the fitted forms are out of their range beyond $s \approx 3$ Å⁻¹, where a computed table is not. **Neutral atoms Z = 1–86 only** — ions are not derivable from it — and, like the fits above, it carries no relativistic $\gamma$.
