...
!!! note "Group Relations… is an actively developed feature"
    The **Group Relations…** browser (A4.2) already computes *translationengleiche* (t-) subgroups/supergroups, and *klassengleiche* (k-) *subgroups*, directly from the space group's own symmetry operations (not from a pre-tabulated list), so every relation shown is independently verified rather than copied from a table. Some branches — k-*supergroups* and *isomorphic* subgroups — are still placeholders in the current version; A4.2 says exactly which tabs and branches are live today.
