...
The elastic factor $f_{e,k}$ in the integrand is built from two sources. For **neutral atoms with $Z = 1\text{–}86$** it is, for $0 \le s \le 6$ Å⁻¹, the first-principles $f_e$ of **Temari** dataset-factors v2.0.0 ([DOI 10.5281/zenodo.22820415](https://doi.org/10.5281/zenodo.22820415); see the [scattering-factor page](../a2-beam-interaction/scattering-factor.md)), and beyond $s = 6$ Å⁻¹ the **Mott–Bethe relation** evaluated directly with the Waasmaier–Kirfel X-ray factor $f_0(s)$; **the part beyond 6 Å⁻¹ is not a Temari value**. The two are joined at $s = 6$ Å⁻¹ without matching (the step there is at most 0.041 %). For **ions** and $Z = 87\text{–}98$, which Temari does not cover, the low-$s$ part is the **Peng 5-Gaussian fit**, blended smoothly into the same Mott–Bethe tail between $1.5$ and $2.5$ Å⁻¹. A tail is needed at all because a Gaussian sum decays much too fast beyond $s \gtrsim 2$ Å⁻¹ (Gaussian tails vanish exponentially, whereas the true $f_e$ has a $1/s^2$ tail), which would underestimate backscattering by many orders of magnitude at $s$ of order $k_{vac}$ (about 40 Å⁻¹ at 200 kV). Two implementation details: $f_0(s)$ is clamped to $[0, Z]$ (some fitted forms diverge outside their fitting range), and the neutral-atom $f_0$ is used even for ionic entries (at high $s$ only the nuclear charge is seen, not the electron cloud).

!!! note "How much the Mott–Bethe tail matters"
    The part beyond $s = 6$ Å⁻¹ contributes 1.5–4.8 % of $f'(0)$ (80–300 kV, $B = 0.3\text{–}1.0$ Å², Si/Fe/Au, $|\mathbf g|/2 \le 25$ nm⁻¹; 1.3–3.7 % at 20 kV). Replacing the tail model changes $f'$ by only 0.04–0.43 %, and the simulated observables (CBED, STEM, HRTEM, EBSD) by at most 0.45 %. Using Temari instead of the Peng fit below 6 Å⁻¹ changes $f'(0)$ by 0.06–0.27 %. The Temari tables are computed values published without a certified error bound (`certification_status = not_certified`).
