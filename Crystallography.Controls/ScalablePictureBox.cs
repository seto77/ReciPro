...
namespace Crystallography.Controls
{
    ...
    public partial class ScalablePictureBox : UserControl
    {
        ...
        /// </summary>
        public RectangleD DrawingArea
        { get { return PseudoBitmap.GetDrawingArea(Center, Zoom, pictureBox.ClientSize); } }
        ...
        private void pictureBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (showFocusRectangle)
            {
                ...
                e.Graphics.FillRectangle(b, pictureBox.ClientRectangle);
            }
            if (ShowAreaRectangle)
            {
                ...
                foreach (var s in Symbols)
                {
                    if (s.SymbolVisible)
                    {
                        ...
                        if (s.Shape == SymbolShape.Cross || s.Shape == SymbolShape.CircleAndCross)
                        {
                            ...
                            {
                                e.Graphics.DrawLine(pen2, new PointF(pt.X - s.CrossSize + 1, pt.Y - s.CrossSize - 1), new PointF(pt.X + s.CrossSize + 1, pt.Y + s.CrossSize - 1));
                                e.Graphics.DrawLine(pen2, new PointF(pt.X - s.CrossSize - 1, pt.Y - s.CrossSize + 1), new PointF(pt.X + s.CrossSize - 1, pt.Y + s.CrossSize + 1));
                                e.Graphics.DrawLine(pen2, new PointF(pt.X + s.CrossSize - 1, pt.Y - s.CrossSize - 1), new PointF(pt.X - s.CrossSize - 1, pt.Y + s.CrossSize - 1));
                                e.Graphics.DrawLine(pen2, new PointF(pt.X + s.CrossSize + 1, pt.Y - s.CrossSize + 1), new PointF(pt.X - s.CrossSize + 1, pt.Y + s.CrossSize + 1));

                                e.Graphics.DrawLine(pen1, new PointF(pt.X - s.CrossSize, pt.Y - s.CrossSize), new PointF(pt.X + s.CrossSize, pt.Y + s.CrossSize));
                                e.Graphics.DrawLine(pen1, new PointF(pt.X + s.CrossSize, pt.Y - s.CrossSize), new PointF(pt.X - s.CrossSize, pt.Y + s.CrossSize));
                                ...
                                if (s.LabelVisible)
                                {
                                    ...
                                    if (s.Bold)
                                    {
                                        e.Graphics.FillPath(new SolidBrush(s.CrossColor2), gp);
                                        e.Graphics.DrawPath(pen1, gp);
                                    }
                                    else
                                    {
                                        e.Graphics.FillPath(new SolidBrush(s.CrossColor1), gp);
        ...
        {
            Bitmap bmp = new Bitmap(pictureBox.Image.Width, pictureBox.Height, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
            Graphics g = Graphics.FromImage(bmp);
            g.DrawImage(pictureBox.Image, new Point(0, 0));
            // return PseudoBitmap.GetImage(
            return bmp;
        }
