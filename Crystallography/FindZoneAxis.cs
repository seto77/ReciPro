                ...
                for (int i = 0; i < zoneAxes.Count; i++)
                    for (int j = i + 1; j < zoneAxes.Count; j++)
                        if (zoneAxes[i].Za1 == zoneAxes[j].Za1)
                        {
                            ...
                            SymmetryStatic.IsRootIndex((plane1.h, plane1.k, plane1.l), cry.Symmetry, out var indices1, true);
                            SymmetryStatic.IsRootIndex((plane2.h, plane2.k, plane2.l), cry.Symmetry, out var indices2, true);
