...
### Inner-shell ionization tables (embedded binary resource, STEM-EDX)

`Crystallography.IonizationFsE0.bin` holds the ionization form factors F(s, E₀)
that the STEM-EDX and ALCHEMI paths use. It is **not third-party data**, but its
provenance is worth stating plainly because the physics is documented elsewhere.

| Item | Source | Status |
|------|--------|--------|
| Ionization form factors F(s, E₀) | Generated by **[Temari](https://github.com/seto77/Temari)** (MIT, same author), an independent open-source project that solves the isolated atom from first principles — self-consistent field, bound orbitals, distorted continuum waves. Documentation: https://seto77.github.io/Temari/ | ReciPro embeds the packed table and does not recompute it at run time. The prescription, its verification and its accuracy are Temari's to state; the dataset generation and its model id are recorded in `tools/IonizationGen/handout/prod_v4_jl/MANIFEST.md`. No Oxley–Allen (2000) table or µSTEM data is contained in the shipped resource. |
