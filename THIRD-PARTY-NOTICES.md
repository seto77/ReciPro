...
Last reviewed: 2026-09-18 (260918Cl: Temari dataset-factors updated to **v2.0.0** (DOI 10.5281/zenodo.22820415, every table computed / not_certified). 260818Cl: dataset-factors v1.0.0 (f_x / f_e) added as a second CC BY 4.0 embedded resource — see "Atomic scattering factor tables" below).
...
**Update 2026-06-12 (win-arm64):** an experimental native ARM64 portable package (`ReciPro-v.X-arm64.zip`) is now published on the GitHub Releases page alongside the unchanged x64 artifacts. Because no upstream win-arm64 binaries exist for GLFW (the `OpenTK.redist.glfw` NuGet package has no win-arm64 asset; GLFW publishes no official arm64 Windows binaries) or for xraylib (conda-forge / vcpkg / MSYS2 all lack win-arm64), the win-arm64 `glfw3.dll` and `libxrl-11.dll` are **built from unmodified upstream release sources by this repository's CI** (`.github/workflows/build-native-deps-arm64.yml`, windows-11-arm runner, static CRT `/MT`) and committed under `ReciPro\native\win-arm64\` together with provenance files (`glfw3.arm64.PROVENANCE.txt`, `libxrl-11.arm64.PROVENANCE.txt`: source tarball URL + SHA256, build commands, output SHA256, Actions run URL). They remain third-party components under their upstream licenses. Both portable ZIPs (win-x64 and win-arm64) are single-file publishes: the managed third-party assemblies and the bundled `libxrl-11.dll` / `glfw3.dll` are embedded inside `ReciPro.exe` (extracted to a per-user cache at first run) rather than shipped as loose files. Both MSI variants are framework-dependent WiX packages. <!-- 260715Ch -->

## NuGet / managed libraries

These ship as managed assemblies inside the MSI (packaged through the ReciPro publish output) and are built from their respective upstream NuGet packages. No central package-version management is in use (`Directory.Packages.props` / `nuget.config` are absent); package versions are pinned inline per project and are consistent across projects for shared packages. The tracked `Directory.Build.props` only supplies a common compiler-warning suppression and does not manage package versions. <!-- 260715Ch -->
...
| Name | Version | Purpose | Bundled or downloaded | Upstream URL | License | Signed by ReciPro? | Notes |
|------|---------|---------|-----------------------|--------------|---------|--------------------|-------|
| Eigen | 5.0.1-dev+master snapshot, upstream commit `17d8ddbd` (2026-09-03); updated 2026-09-05 (260905Cl) | Linear algebra (compiled into `Crystallography.Native.dll`) | Bundled as vendored header-only source; compiled in, not shipped as a separate library | https://eigen.tuxfamily.org/ | MPL-2.0 | N/A (compiled into a ReciPro-built native DLL; the DLL is not individually signed) | Version from `Eigen/Version` (WORLD 3, MAJOR 5, MINOR 0, PATCH 1; the snapshot commit and archive sha256 are recorded in `Crystallography.Native/THIRD-PARTY-NOTICES.md`). Preserve the MPL-2.0 notice. Optional LAPACKE/MKL Eigen backends in the header tree are inert (`EIGEN_USE_MKL` / `EIGEN_USE_BLAS` / `EIGEN_USE_LAPACKE` are never defined in the project sources). <!-- 260715Ch --> |
...
### Inner-shell ionization tables (embedded binary resource, STEM-EDX and ALCHEMI)

`Crystallography.IonizationFsE0.bin` holds the ionization form factors F(s, E₀)
that the STEM-EDX and ALCHEMI paths use. It is generated by the same author, but
**it is not covered by ReciPro's MIT licence.** Temari publishes its software under
MIT and the datasets it generates under **CC BY 4.0**, deliberately and separately,
because MIT is written for software and is silent on the EU database right
(see `licenses/README.md` in the Temari repository). ReciPro redistributes a
*modified* form of that dataset, so the CC BY 4.0 attribution below applies.
<!-- 260811Cl 全面改稿: 旧版は「not third-party data」と書き、Temari を MIT とだけ記していた。
     ソフトは MIT でもデータは CC BY 4.0 で、ReciPro はその改変版を再配布している。 -->
...
ReciPro embeds the packed table and does not recompute it at run time. The
prescription, its verification and its stated accuracy are **Temari's to state**,
not ReciPro's — see https://seto77.github.io/Temari/ .
...
| Item | Detail |
|------|--------|
| Material | Yusuke Seto, *Atomic scattering factors f_x(s) and f_e(s) for 86 neutral atoms (Z = 1–86) computed with Temari*, dataset **dataset-factors v2.0.0** (2026). <!-- 260918Cl v1.0.0 → v2.0.0 --> |
| Creator and copyright | © 2026 Yusuke Seto. |
| Licence | **CC BY 4.0** — https://creativecommons.org/licenses/by/4.0/ |
| Link to the original | DOI [10.5281/zenodo.22820415](https://doi.org/10.5281/zenodo.22820415) (this version; the version-independent DOI 10.5281/zenodo.22644247 resolves to the latest). Release [`dataset-factors-v2.0.0`](https://github.com/seto77/Temari/releases/tag/dataset-factors-v2.0.0), archive SHA-256 `0f6f7182…8d70a`. Generator and documentation: https://github.com/seto77/Temari , https://seto77.github.io/Temari/ |
| **Changes made** | **Yes.** All 7681 nodes are kept and the published interpolation contract is followed, but the values are **requantized** to an absolute step of 1e-10 electrons (f_x) and 1e-11 Å (f_e) before being delta-coded and Brotli-compressed (`tools/TemariFactorsGen/pack_temari_factors.py`). The largest deviation from the published values is 5e-11 / 5e-12 — one tenth of the 11-significant-digit rounding Temari itself applied on release. This is a conversion difference from the published values, not a statement about their physical accuracy (see the next row). Nothing was recomputed. |
| Disclaimer | The dataset is offered as-is, without warranties or conditions of any kind (CC BY 4.0 §5). |
...
Since v.4.942, SignPath signs the x64 WiX MSI container and the x64 portable single-file `ReciPro.exe` before release. The framework-dependent executable and DLLs inside the MSI are not individually signed; their integrity is covered by the signed MSI container. The loose `Crystallography.Native*.dll` files in the x64 portable ZIP are also not individually signed. The experimental Arm64 MSI, portable executable, and native DLLs currently remain unsigned. In both portable packages, third-party libraries embedded inside the single-file bundle retain their own provenance and are not individually re-signed. See `CODE_SIGNING.md` for the release-artifact signing policy. This notice is not a substitute for the upstream license texts — refer to each upstream project or data provider for authoritative terms. <!-- 260715Ch: verified against release.yml and v.4.943 artifacts -->
